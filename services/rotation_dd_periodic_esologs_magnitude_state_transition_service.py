...
class RotationDDPeriodicEsoLogsMagnitudeStateTransitionService:
    """Correlate same-cast periodic amount changes with observed buff/debuff changes.

    Canonical lower-snake identity selects cast anchors. Numeric periodic/effect IDs
    remain ESO Logs evidence handles only. A state event is considered relevant when
    it targets either the damage source or the damage target between two consecutive
    same-cast, same-target, same-hit-type periodic events. Results remain observational
    and never promote snapshot-vs-dynamic magnitude policy automatically.
    """
    ...
    def inspect(
        ...
        *,
        ...
        if not identity:
            return self._report("", periodic_ability_id, unresolved=("canonical skill identity is required",))
        ...
        if int(periodic_ability_id) <= 0:
            return self._report(identity, periodic_ability_id, unresolved=("positive observational periodic ability ID is required",))
        ...
        with self._open_logs() as db:
            ...
            if schema_error:
                return self._report(identity, periodic_ability_id, unresolved=tuple(dict.fromkeys((*unresolved, schema_error))))

            casts = tuple(
                row
                for row in self._cast_rows(db, report_code=report_code, fight_id=fight_id, source_id=source_id)
            ...
            if not casts:
                ...
                return self._report(identity, periodic_ability_id, unresolved=tuple(dict.fromkeys(unresolved)))

            anchor_type = next(kind for kind in _CAST_TYPES if any(self._event_type(row) == kind for row in casts))
            anchor_tracks: set[tuple[str, int, int, int]] = set()
            groups: set[tuple[str, int, int]] = set()
            for row in casts:
                if self._event_type(row) != anchor_type or row["source_id"] is None or row["cast_track_id"] is None:
                    continue
            ...
            for group_report, group_fight, group_source in groups:
                ...
                for row in periodic_rows:
                    if row["cast_track_id"] is None or row["target_id"] is None or row["amount"] is None:
                        continue
                    track = int(row["cast_track_id"])
                    if (group_report, group_fight, group_source, track) not in anchor_tracks:
                        continue
                    key = (track, int(row["target_id"]), None if row["hit_type"] is None else int(row["hit_type"]))
                ...
                for (track, target, hit_type), rows in grouped.items():
                    ...
                    for previous, current in zip(ordered, ordered[1:]):
                        ...
                        if from_amount == to_amount:
                            continue
                        state_rows = self._state_rows_between(
                            db,
                            report_code=group_report,
                            fight_id=group_fight,
                            relevant_targets=(group_source, target),
                            start_timestamp=float(previous["timestamp"]),
                            start_event_index=int(previous["event_index"]),
                            end_timestamp=float(current["timestamp"]),
                            end_event_index=int(current["event_index"]),
                        )
                        transitions.append(
                            RotationDDPeriodicEsoLogsMagnitudeTransition(
                                ...
                                cast_track_id=track,
                                hit_type=hit_type,
                                from_timestamp_ms=float(previous["timestamp"]),
                                to_timestamp_ms=float(current["timestamp"]),
                                from_amount=from_amount,
                                to_amount=to_amount,
                                state_events=tuple(self._state_evidence(row) for row in state_rows),
                            )
                        )

        if not transitions:
            unresolved.append(f"{identity}: no same-cast periodic amount-change transitions were observed")
        return self._report(
            identity,
            periodic_ability_id,
    ...
    def _numeric_aliases(self, skill_id: int, morph: int, base_ability_id: int) -> tuple[int, ...]:
        ...
        with sqlite3.connect(uri, uri=True) as db:
            ...
            if db.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='skill_rank'").fetchone() is not None:
                aliases.update(
                    int(row["ability_id"])
    ...
    def _schema_error(db: sqlite3.Connection) -> str | None:
        if db.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='log_event'").fetchone() is None:
            return "log_event table is unavailable"
        required = {"report_code", "fight_id", "event_index", "timestamp", "event_type", "source_id", "target_id", "ability_game_id", "amount", "hit_type", "cast_track_id", "raw_json"}
    ...
    @staticmethod
    def _cast_rows(db: sqlite3.Connection, *, report_code: str | None, fight_id: int | None, source_id: int | None) -> tuple[sqlite3.Row, ...]:
        clauses = ["lower(event_type) IN ('cast','begincast','completecast')"]
        params: list[object] = []
        if report_code is not None:
            clauses.append("report_code=?"); params.append(str(report_code))
        if fight_id is not None:
            clauses.append("fight_id=?"); params.append(int(fight_id))
        if source_id is not None:
            clauses.append("source_id=?"); params.append(int(source_id))
        return tuple(db.execute(
            "SELECT report_code,fight_id,event_index,timestamp,event_type,source_id,ability_game_id,cast_track_id,raw_json FROM log_event WHERE " + " AND ".join(clauses) + " ORDER BY report_code,fight_id,source_id,timestamp,event_index",
            tuple(params),
        ).fetchall())

    @staticmethod
    def _periodic_rows(db: sqlite3.Connection, *, report_code: str, fight_id: int, source_id: int, periodic_ability_id: int) -> tuple[sqlite3.Row, ...]:
        return tuple(db.execute(
            "SELECT report_code,fight_id,event_index,timestamp,event_type,source_id,target_id,ability_game_id,amount,hit_type,cast_track_id,raw_json FROM log_event WHERE report_code=? AND fight_id=? AND source_id=? AND ability_game_id=? AND amount IS NOT NULL ORDER BY timestamp,event_index",
            (report_code, int(fight_id), int(source_id), int(periodic_ability_id)),
        ).fetchall())

    @staticmethod
    def _state_rows_between(
        db: sqlite3.Connection,
        *,
        report_code: str,
        fight_id: int,
        relevant_targets: tuple[int, int],
        start_timestamp: float,
        start_event_index: int,
        end_timestamp: float,
        end_event_index: int,
    ) -> tuple[sqlite3.Row, ...]:
        placeholders = ",".join("?" for _ in _STATE_EVENT_TYPES)
        return tuple(db.execute(
            f"SELECT report_code,fight_id,event_index,timestamp,event_type,source_id,target_id,ability_game_id,raw_json FROM log_event WHERE report_code=? AND fight_id=? AND lower(event_type) IN ({placeholders}) AND target_id IN (?,?) AND ((timestamp>? AND timestamp<?) OR (timestamp=? AND event_index>?) OR (timestamp=? AND event_index<?)) ORDER BY timestamp,event_index",
            (
                report_code,
                int(fight_id),
                *_STATE_EVENT_TYPES,
                int(relevant_targets[0]),
                int(relevant_targets[1]),
    ...
    @classmethod
    def _state_evidence(cls, row: sqlite3.Row) -> RotationDDPeriodicEsoLogsStateEventEvidence:
        return RotationDDPeriodicEsoLogsStateEventEvidence(
            ...
            ability_game_id=None if row["ability_game_id"] is None else int(row["ability_game_id"]),

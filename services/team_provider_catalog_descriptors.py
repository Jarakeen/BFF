...
TEAM_PROVIDER_SERVICE_DESCRIPTORS: tuple[ServiceDescriptor, ...] = (
    ...
    ServiceDescriptor(
        service_id="team.provider.assignment_workload_adapter",
        domain="team",
        purpose="Project explicit encounter provider ownership and rotation-effect policy through exact saved plan casts into canonical provider workload candidate requests.",
        implementation_path="services.team_provider_assignment_workload_adapter_service",
        inputs=("ProviderAssignment", "RotationAssignmentEffectPolicy", "TeamProviderAssignedWorkloadPolicy", "RotationPlan"),
        outputs=("TeamProviderAssignmentWorkloadProjection", "TeamProviderWorkloadAlternativeRequest"),
        dependencies=("encounter.provider.assignment", "team.provider.coverage", "team.provider.temporal_coverage"),
        responsibilities=("team_provider_assignment_workload_projection",),
        behavior=ServiceBehavior.DETERMINISTIC,
        encounter_aware=True,
        evidence_class=EvidenceClass.MIXED,
        notes="Provider ownership is never re-selected here. Effect/source/uptime semantics remain owned by explicit rotation assignment policy; workload-only duration, target capacity, horizon, and displacement facts must be supplied explicitly.",
    ),
    ServiceDescriptor(

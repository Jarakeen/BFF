...
Racial resource passives are reconciled against ``RaceRepository`` because their
structured stat contribution is already applied by the canonical character-state
pipeline. They are reported as accounted elsewhere rather than searched again.
    ...
    def build(self, objective_key: str) -> ExtremeResourcePassiveCoverageAudit:
        ...
        for passive in passives:
            ...
            if projection.status is ExtremePassiveProjectionStatus.REVIEWED_STATIC:
                ...
                if relevant and self._racial_resource_already_accounted(passive, key):

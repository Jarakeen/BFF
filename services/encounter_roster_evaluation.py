...
class EncounterRosterEvaluationReport:
    ...
    def is_fully_evaluable(self) -> bool:
        provider_evaluable = all(
            row.classification.value != "unknown" for row in self.provider_results
        )
    ...
    def is_fully_covered(self) -> bool:
        ...
        return self.is_fully_evaluable and provider_covered and self.execution_evaluation.is_fully_ready


class EncounterRosterEvaluator:
    """Compose Phase 9 demands with multiple canonical capability-evidence families."""
    ...
    def evaluate_saved_build_audits(
        ...
        if len(roster_members) != len(set(roster_members)):
            raise ValueError(
                "saved-build roster must resolve to unique member identities; select one authoritative build per roster member"
            )
        ...
        owned_types = getattr(self._adapter, "mapped_capability_types", provider_capabilities)
        effect_capabilities = tuple(
            capability for capability in provider_capabilities if capability in set(owned_types)
        )
        effect_evidence = self._adapter.evidence_for(audits, effect_capabilities)

        additional = tuple(additional_capability_evidence)
        roster_set = set(roster_members)
        foreign_members = tuple(row.member_id for row in additional if row.member_id not in roster_set)
        if foreign_members:
            raise ValueError(
        ...
        requirement_evaluation = self._evaluator.evaluate(encounter_id, roster_members, evidence)
        return EncounterRosterEvaluationReport(
            requirement_evaluation=requirement_evaluation,
            execution_evaluation=self._execution_evaluator.evaluate(encounter_id, selected_difficulty),

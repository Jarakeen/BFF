...
class ExtremeSustainedDPSWeaponFrontierService:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(enchant_choices, tuple):
            raise TypeError("weapon enchant choices must be a tuple")
        if trait_choices is not None and not isinstance(trait_choices, tuple):
            raise TypeError("weapon trait choices must be a tuple")
        self.enchant_choices = self._clean(enchant_choices)
        self.trait_choices = self._clean(
            trait_choices if trait_choices is not None else tuple(WEAPON_TRAITS)
        )
    ...
    def page(
        ...
        *,
        ...
        if isinstance(offset, bool) or not isinstance(offset, int):
            raise TypeError("weapon page offset must be an integer")
        if isinstance(limit, bool) or not isinstance(limit, int):
            raise TypeError("weapon page limit must be an integer")
        start = max(0, offset)
        size = max(0, limit)
    ...
    def _clean(values: tuple[str, ...]) -> tuple[str, ...]:
        if not isinstance(values, tuple):
            raise TypeError("weapon frontier choices must be a tuple")
        if any(not isinstance(value, str) for value in values):
            raise TypeError("weapon frontier choices must contain only strings")
        unique = {

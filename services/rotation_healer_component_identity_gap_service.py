...
class RotationHealerComponentIdentityGapRow:
    bar: str
    slot: int
    skill_name: str
    skill_rank_id: int
    ...
    canonical_heal_temporal_scope: HealTemporalScope | None
    canonical_is_dot: bool | None
    text_evidence: SkillComponentTextEvidence

    @property
    def text_proves_heal(self) -> bool:
        return self.text_evidence.effect_kind == "heal"

    @property
    def text_proves_periodic_heal(self) -> bool:
        return self.text_proves_heal and self.text_evidence.is_dot is True

    @property
    def canonical_proves_heal(self) -> bool:
        return self.canonical_effect_kind is SkillEffectKind.HEAL

    @property
    def canonical_proves_periodic_heal(self) -> bool:
        return self.canonical_proves_heal and (
            self.canonical_heal_temporal_scope is HealTemporalScope.PERIODIC
            or self.canonical_is_dot is True
        )

    @property
    def needs_heal_identity_review(self) -> bool:
        return self.text_proves_heal and not self.canonical_proves_heal

    @property
    def needs_periodic_identity_review(self) -> bool:
        return self.text_proves_periodic_heal and not self.canonical_proves_periodic_heal


@dataclass(frozen=True)
class RotationHealerComponentIdentityGapReport:
    character_name: str
    build_name: str
    rows: tuple[RotationHealerComponentIdentityGapRow, ...]
    unresolved: tuple[str, ...] = ()

    @property
    def review_candidates(self) -> tuple[RotationHealerComponentIdentityGapRow, ...]:
        return tuple(
            row
            for row in self.rows
            if row.needs_heal_identity_review or row.needs_periodic_identity_review
        )
        ...
        for bar, skills in (("front", build.FrontBarSkills), ("back", build.BackBarSkills)):
            for slot, raw_name in enumerate(skills, start=1):
                ...
                if resolution.rank is None:
                    messages = resolution.unresolved or (f"canonical skill identity unresolved for {skill_name}",)
                    unresolved.extend(f"{bar} slot {slot} {skill_name}: {message}" for message in messages)
                    continue
                ...
                if not description:
                    unresolved.append(
                        f"{bar} slot {slot} {rank.name}: coef_description unavailable for component identity audit"
                    )
                    continue
                ...
                for coefficient in rank.coefficients:
                    ...
                    canonical_kind = canonical.effect_kind if canonical is not None else SkillEffectKind.UNKNOWN
                    canonical_temporal = canonical.heal_temporal_scope if canonical is not None else None
                    canonical_is_dot = canonical.is_dot if canonical is not None else None

                    if canonical_kind is not SkillEffectKind.HEAL and text.effect_kind != "heal":
                        continue

                    rows.append(
                        RotationHealerComponentIdentityGapRow(
                            bar=bar,
                            slot=slot,
                            ...
                            canonical_heal_temporal_scope=canonical_temporal,
                            canonical_is_dot=canonical_is_dot,
                            text_evidence=text,
                        )
                    )

        return RotationHealerComponentIdentityGapReport(

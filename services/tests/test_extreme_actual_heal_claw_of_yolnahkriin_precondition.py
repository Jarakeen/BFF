...
def _build(piece_count: int = 5) -> PlayerBuild:
    ...
    for slot in ("Head", "Chest", "Legs", "Shoulders", "Hands")[:piece_count]:
        build.Armor[slot]["Set"] = "Claw of Yolnahkriin"
    return build


def test_claw_of_yolnahkriin_requires_five_pieces_for_minor_courage_witness() -> None:
    ...
    assert CLAW_OF_YOLNAHKRIIN_MINOR_COURAGE_CONDITION not in inactive.condition_context
    assert CLAW_OF_YOLNAHKRIIN_MINOR_COURAGE_CONDITION in active.condition_context
    assert any("taunt one enemy" in item for item in active.evidence)
    assert any("15-second" in item for item in active.evidence)


def test_claw_of_yolnahkriin_routes_through_canonical_minor_courage() -> None:
    state = ExtremeResourceConditionedPhase5ContextFactory._combat_state_with_reviewed_gear_witnesses(
        ...
        frozenset({CLAW_OF_YOLNAHKRIIN_MINOR_COURAGE_CONDITION}),
    )
...
def test_claw_of_yolnahkriin_exact_h1_blocker_is_reviewed_by_taunt_witness() -> None:
    blocker = (
        "Claw of Yolnahkriin (5): active set bonus is not yet mechanic-mapped: "
        "(5 items) When you taunt an enemy, you give yourself and 11 group members Minor Courage "
        "for 15 seconds, increasing your Weapon and Spell Damage by 215. This effect can occur "
        "once every 8 seconds."
    )
    row = ExtremeGearSetObjectiveCandidate(
        set_id=1,
        set_name="Claw of Yolnahkriin",
        category="Test",

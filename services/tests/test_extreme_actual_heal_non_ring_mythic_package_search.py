...
class _HealingEvents:
    def evaluate(self, *, build, context, entity_id):
        _ = context
        score = 430.0 if build.Armor["Legs"]["Set"] == "Legs Mythic" else 100.0
...
class _NonRingMythicPackages:
    ...
    def build_candidates(
        ...
        *,
        ...
        build.Armor["Legs"]["Set"] = "Legs Mythic"
        return (
            ExtremeCompleteOptimizationService._direct_candidate(
                build,
                character_id=character_id,
                baseline_build_id=baseline_build_id,
                token="actual-heal-slot-mythic:legs",
                path="Gear.FivePiecePlusFivePiecePlusSlotMythic",
                before={"mythic_slot": "Legs"},
                after={"mythic": "Legs Mythic", "mythic_slot": "Legs"},
                source="test:slot-mythic",
            ),
        )
...
def test_actual_heal_optimizer_can_select_non_ring_mythic_package(monkeypatch):
    ...
    service = ExtremeActualHealOptimizationService(
        ...
        non_ring_mythic_packages=_NonRingMythicPackages(),
    )
    ...
    assert result.optimized_event.critical_heal == 430.0
    assert result.gain == 330.0
    assert result.steps[-1].path == "Gear.FivePiecePlusFivePiecePlusSlotMythic"

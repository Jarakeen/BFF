...
def _olorime_row() -> ExtremeGearSetObjectiveCandidate:
    blocker = (
...
def test_olorime_exact_h1_blocker_is_reviewed_after_witness_path() -> None:
    ...
    assert result.ignored_blockers == row.unresolved


def test_olorime_specialist_positive_is_authoritatively_admissible_with_zero_shared_delta() -> None:
    row = _olorime_row()

    assert row.reviewed_delta == 0.0
    assert ExtremeActualHealGearSetCandidateService._h1_positive(row) is True
    assert ExtremeActualHealGearSetCandidateService._h1_mechanic_complete(row) is True

...
def test_real_corpus_cleanse_method_audit_tracks_all_current_methods():
    ...
    assert audit.encounters_with_cleanse_requirements >= 2
    assert audit.encounters_with_resolved_methods == audit.encounters_with_cleanse_requirements
    assert audit.encounters_missing_method_detail == 0
    assert audit.resolved_method_count >= audit.encounters_with_cleanse_requirements
    assert (
        audit.encounter_interaction_count
        + audit.core_action_count
        + audit.player_build_method_count
        == audit.resolved_method_count
    )
    ...
    xalvakka = next((row for row in audit.rows if row.encounter_id == "xalvakka"), None)
    if xalvakka is not None and xalvakka.cleanse_requirement_count:
        assert xalvakka.resolved_method_count == xalvakka.cleanse_requirement_count

...
def test_coordinator_adds_context_when_lower_output_and_weaker_contact_coexist() -> None:
    observations = (
    ...
    context = next(item for item in result.report.findings if item.category == "damage_context")
    assert context.subject == "DD One"
    assert "Mechanic-adjusted boss contact was weaker on wipes" in context.evidence
    assert "weaker boss-contact continuity" in context.recommendation


def test_coordinator_does_not_duplicate_lower_output_when_no_context_exists() -> None:
    observations = (
        _obs(1, kill=True, output=100_000),
        _obs(2, kill=True, output=102_000),
        _obs(3, kill=False, output=70_000),
    ...
    assert not any(item.category == "damage_context" for item in result.report.findings)
    assert any(item.category == "damage" for item in result.report.findings)

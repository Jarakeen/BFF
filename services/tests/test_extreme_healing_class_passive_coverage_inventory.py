...
def test_inventory_starts_conservative_and_records_only_proven_reviewed_families() -> None:
    ...
    assert all(row.healing_relevant is True for row in reviewed.values())
    assert all(row.coverage_status == IMPLEMENTED for row in reviewed.values())
    assert all(
        row.review_status == UNREVIEWED
        for row in rows
        if (row.eso_class, row.skill_line) not in reviewed
    )

...
def test_four_reviewed_families_leave_no_partial_families() -> None:
    rows = ExtremeHealingClassPassiveCoverageInventory().items()
    ...
    assert {
        ...
        ("templar", "Restoring Light"): EXPLICITLY_UNSUPPORTED,
        ("warden", "Green Balance"): IMPLEMENTED,
    }
    assert partial == []


def test_inventory_summary_counts_restoring_light_review_and_orchestration_blocker() -> None:
...
def test_actual_heal_audit_reports_reviewed_but_unsupported_restoring_light_family() -> None:

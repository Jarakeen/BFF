...
def _bar_attempt(*, time_seconds: float, active_bar: str, sequence: int = 0) -> ExtremeRuntimeBarEffectAttempt:
...
def test_unified_runtime_history_orders_effects_potions_and_external_buffs_together():
    ...
    assert snapshot.ordered_runtime_history == (potion_use, early_attempt, external_buff, late_attempt)
...
def test_bar_annotated_effect_attempt_preserves_provenance_and_compatibility_view():
    ...
    snapshot = ExtremeRuntimeSnapshot(runtime_history=(back, unbarred, front), snapshot_time_seconds=5.0)
...
def test_snapshot_at_preserves_bar_provenance_and_sequence_boundary():
    ...
    source = ExtremeRuntimeSnapshot(runtime_history=(back_after, front, back_before), snapshot_time_seconds=20.0)
...
def test_snapshot_at_slices_future_bar_transitions_but_preserves_completeness_proof():
    early = ExtremeRuntimeBarTransition(5.0, 0, "front", "back")
    future = ExtremeRuntimeBarTransition(12.0, 0, "back", "front")
    source = ExtremeRuntimeSnapshot(
        runtime_history=(future, early),
        snapshot_time_seconds=20.0,
        bar_transition_history_complete=True,
    )
    sliced = source.snapshot_at(10.0)
    assert sliced.bar_transitions == (early,)
...
def test_invalid_bar_provenance_fails_closed():
    ...
    with pytest.raises(ValueError, match="runtime bar transition must change"):
        ExtremeRuntimeBarTransition(1.0, 0, "front", "front")


def test_unified_runtime_history_uses_latest_potion_activation_before_snapshot():
    snapshot = ExtremeRuntimeSnapshot(
        runtime_history=(ExtremeRuntimePotionUse(time_seconds=1.0), ExtremeRuntimePotionUse(time_seconds=7.5)),
        snapshot_time_seconds=10.0,
    )
...
def test_unified_runtime_history_ignores_future_potion_activation():
    ...
    snapshot = ExtremeRuntimeSnapshot(runtime_history=(future_potion,), snapshot_time_seconds=10.0)
...
def test_unified_runtime_history_rejects_mixed_legacy_inputs():
    with pytest.raises(ValueError, match="runtime_history cannot be combined with legacy attempts or potion_elapsed_seconds"):
        ExtremeRuntimeSnapshot(
...
def test_legacy_runtime_inputs_remain_supported_during_migration():
    ...
    snapshot = ExtremeRuntimeSnapshot(attempts=(attempt,), snapshot_time_seconds=5.0, potion_elapsed_seconds=3.0)
...
def test_runtime_snapshot_at_refuses_to_time_shift_legacy_evidence():
    snapshot = ExtremeRuntimeSnapshot(snapshot_time_seconds=10.0, potion_elapsed_seconds=4.0)

...
def test_checked_in_registry_keeps_reviewed_progression_counts_and_chronology():
    ...
    expected_counts = {
        "naj_caldeesh": 3, "black_gem_foundry": 3, "exiled_redoubt": 3,
        "lep_seclusa": 3, "oathsworn_pit": 3, "bedlam_veil": 3,
        "bal_sunnar": 3, "scrivener_s_hall": 3, "earthen_root_enclave": 3,
        "graven_deep": 3, "coral_aerie": 3, "shipwright_s_regret": 3,
        "red_petal_bastion": 3, "the_dread_cellar": 3, "black_drake_villa": 3,
        "the_cauldron": 4, "castle_thorn": 5, "stone_garden": 3,
        "icereach": 5, "unhallowed_grave": 5, "moongrave_fane": 5,
        "lair_of_maarselok": 5, "depths_of_malatar": 5, "frostvault": 5,
        "moon_hunter_keep": 5, "march_of_sacrifices": 5, "fang_lair": 5,
        "scalecaller_peak": 5, "bloodroot_forge": 6, "falkreath_hold": 5,
        "cradle_of_shadows": 5, "ruins_of_mazzatun": 4,
        "imperial_city_prison": 6, "white_gold_tower": 4,
        "city_of_ash_ii": 3, "crypt_of_hearts_ii": 3,
        "fungal_grotto_ii": 3, "spindleclutch_ii": 3,
        "the_banished_cells_ii": 3, "darkshade_caverns_ii": 3,
...
def test_checked_in_registry_preserves_grouped_encounter_identities():
    data_root = Path(__file__).resolve().parents[2] / "data"
    by_id = {row.encounter_id: row for row in load_dungeon_encounter_identities(data_root)}

    assert by_id["talen_lah"].member_ids == ("talen_lah", "bar_sakka")
    assert by_id["artifact_bearers"].member_ids == ("eliam_merick", "ihudir", "liramindrel")
    ...
    assert by_id["wyrd_sisters"].member_ids == (
        "wyress_rangifer", "wyress_strigidae", "wyress_ursus"
    )
    assert by_id["cadaverous_menagerie"].member_ids == (
        "cadaverous_bear", "cadaverous_guar", "cadaverous_senche_tiger"
    )
    ...
    assert by_id["allene_pellingare_wayrest_sewers_i"].member_ids == (
        "allene_pellingare",
    )
...
def test_launch_veteran_slice_contains_only_conqueror_progression_encounters():
    ...
    assert by_content["fungal_grotto_ii"] == {"gamyne_bandu", "spawn_of_mephala", "vila_theran"}
    assert by_content["spindleclutch_ii"] == {"bloodspawn_creature", "praxin_douare", "vorenor_winterbourne"}
    ...
    assert by_content["elden_hollow_ii"] == {"dark_root", "murklight", "bogdan_the_nightflame"}
    ...
    assert not {
        "dunmer", "spider_daedra", "gargoyle", "wraith", "fighters_guild", "nord",
        "draining_scrib", "sedating_scrib", "netch", "dwemer_spider", "spriggan",
        "lurcher", "daedric_titan", "harvester", "lich",
    } & all_ids
...
def test_launch_starter_slice_contains_only_vanquisher_progression_encounters():
    ...
    assert by_content["fungal_grotto_i"] == {"war_chief_ozozai", "kra_gh_the_dreugh_king"}
    assert by_content["spindleclutch_i"] == {"swarm_mother", "the_whisperer"}
    assert by_content["the_banished_cells_i"] == {
        "shadowrend", "high_kinlord_rilis_banished_cells_i"
    }
    assert by_content["darkshade_caverns_i"] == {
        "foreman_llothan", "the_hive_lord", "sentinel_of_rkugamz"
    }
    assert by_content["elden_hollow_i"] == {
...
def test_known_reviewed_semantic_import_gaps_stay_explicit():
    data_root = Path(__file__).resolve().parents[2] / "data"
    all_ids = {row.encounter_id for row in load_dungeon_encounter_identities(data_root)}
    known_missing_raw = {
        "icestalker", "lizabet_charnis", "doylemish_ironheart", "mathgamain",
        "stoneheart", "gherig_bullblood", "morrigh_bullblood",
        "deathlord_bjarfrud_skjoralmor", "sithera", "votary_of_velidreth",
        "zatzu_the_spine_breaker", "overfiend", "gravelight_sentry",
        "lord_wardens_council", "elite_guard",
    }

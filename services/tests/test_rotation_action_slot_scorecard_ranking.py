...
def _scorecard(plan: RotationPlan) -> RotationCandidateScorecard:
    requirement = RotationActionSlotRequirement("Front Heal", ("front",))
...
def test_wrong_slotted_bar_is_hard_ineligible_even_when_candidate_id_would_win_tie() -> None:
    illegal = RotationCandidateRankingInput("aaa-illegal", _scorecard(_plan("back")))
    legal = RotationCandidateRankingInput("zzz-legal", _scorecard(_plan("front")))
    ...
    assert any(
        "scheduled back bar, slotted on front" in reason

...
def _build(piece_count: int = 5) -> PlayerBuild:
    ...
    for slot in ("Head", "Chest", "Legs", "Shoulders", "Hands")[:piece_count]:
        build.Armor[slot]["Set"] = "Stygian"
    return build


def test_stygian_requires_five_pieces_for_sneak_exit_witness() -> None:
    ...
    assert STYGIAN_POWER_CONDITION not in inactive.condition_context
    assert STYGIAN_POWER_CONDITION in active.condition_context
    assert any("leave Sneak" in item for item in active.evidence)


def test_stygian_maps_conditional_power_window() -> None:
    bonus = GearSetBonus(
        id=1,
        set_id=1,
        piece_count=5,
        description=(
            "(5 items) When you leave Sneak or invisibility while in combat, your Weapon and "
            "Spell Damage is increased by 369 for 15 seconds."
        ),
    )
    ...
    assert {effect.condition for effect in effects} == {STYGIAN_POWER_CONDITION}


def test_stygian_h1_blocker_is_reviewed_by_sneak_exit_witness() -> None:
    blocker = (
        "Stygian (5): active set bonus is not yet mechanic-mapped: "
        "(5 items) When you leave Sneak or invisibility while in combat, your Weapon and Spell "
        "Damage is increased by 369 for 15 seconds."
    )

...
def test_illustrious_healing_has_reviewed_two_second_cadence():
    ...
    assert result.timing.source == "reviewed_zos_patch_history_u50"
    assert any("v8.1.0" in item for item in result.provenance)


def test_echoing_vigor_has_reviewed_two_second_cadence():
    result = RotationHealerU50PeriodicCadenceRepository().get(
    ...
    assert result.timing.interval_seconds == 2.0
    assert result.timing.bound_kind is RuntimeCadenceBoundKind.CALLER_ACTIVE_WINDOW
    assert any("v5.1.0" in item for item in result.provenance)
...
def test_radiating_regeneration_remains_unreviewed_until_exact_cadence_is_proven():
    result = RotationHealerU50PeriodicCadenceRepository().get(

...
def test_runtime_target_state_factory_binds_to_final_stabilized_plan() -> None:
    final_plan = RotationPlan(
        character_name="Target Test",
        build_name="final-target-plan",
    ...
    def evaluate_final_family(snapshots):
        ...
        assert snapshot.runtime_target_combat_state_resolver is not None
        state = snapshot.runtime_target_combat_state_resolver(4.0, 2)
        assert state.has_buff("Major Vulnerability")
        return (
            SimpleNamespace(
                candidate_id="target-state",
    ...
    result = RotationRecoveryHeavyCandidateOrchestrationService(
        ...
        build=PlayerBuild(Name="Target Test", BuildName="DD"),
        ...
        runtime_target_combat_state_resolver_factory=target_factory,
    )

    assert factory_calls == [final_plan]
    assert resolver_calls == [(final_plan, 4.0, 2)]
    assert result.stabilized_candidates[0].runtime_target_combat_state_resolver is not None

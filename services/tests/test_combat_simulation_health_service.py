...
def test_raw_non_finite_outgoing_damage_fails_closed_as_damage_unresolved() -> None:
    state = CombatSimulationTargetState(
        combatants=(
            CombatSimulationCombatant(
                "Boss",
    ...
    for amount in (float("nan"), float("inf"), float("-inf")):
        ...
        assert result.events == ()
        assert result.damage_unresolved
        assert any("damage amount must be finite" in message for message in result.damage_unresolved)


def test_raw_non_finite_heal_fails_closed_without_damage_unresolved() -> None:
    state = CombatSimulationTargetState(
        combatants=(
            CombatSimulationCombatant(
                "Tank 1",

...
class ExtremeResourceMundusProjectionService:
    ...
    def build(self, objective_key: str) -> ExtremeResourceMundusProjection:
        ...
        names = tuple(str(name or "").strip() for name in self.repository.list_names() if str(name or "").strip())
        unresolved: list[str] = []
        positive: list[tuple[float, str]] = []

        for name in names:
            records = tuple(self.repository.get_records(name))
            if not records:
                unresolved.append(f"Mundus stone has no canonical records: {name}")
                continue

            target_records = tuple(row for row in records if str(row.stat_id).strip().casefold() == target.casefold())
            ...
            if not bool(row.supported):
                unresolved.append(f"Mundus stone has unsupported {key} semantics: {name}: {row.notes}")
                continue
            if str(row.unit or "").strip().casefold() != "flat":
                unresolved.append(f"Mundus stone has non-flat {key} semantics: {name}: {row.unit}")
                continue
            value = float(row.value)
            if value > 0.0:
                positive.append((value, name))
            elif value != 0.0:
                unresolved.append(f"Mundus stone has non-positive {key} mutation: {name}: {value}")

        witness: str | None = None
        if positive:
            positive.sort(key=lambda item: (-item[0], item[1].casefold(), item[1]))
            best_value = positive[0][0]
            tied = tuple(name for value, name in positive if abs(value - best_value) <= 1e-9)
            if len(tied) == 1:
                witness = tied[0]
            else:
                unresolved.append(
            ...
            unresolved.append(f"Canonical Mundus catalogue contains no positive {key} witness")

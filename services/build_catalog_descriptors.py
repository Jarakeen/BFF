...
BUILD_SERVICE_DESCRIPTORS: tuple[ServiceDescriptor, ...] = (
    ...
    ServiceDescriptor(
        ...
        domain="build",
        purpose="Persist the latest explicitly completed RotationPlan for a canonical saved build without mutating ESO reference data or regenerating rotation evidence.",
        ...
        outputs=("BuildRotationArtifact",),
        dependencies=("build.catalog.persistence",),
        responsibilities=("build_owned_rotation_artifact_persistence",),
        behavior=ServiceBehavior.DETERMINISTIC,
        ui_safe=True,
        evidence_class=EvidenceClass.MIXED,
        notes="Artifacts are keyed by canonical build_id in build_rotations.json. Saving copies only an already-rendered completed plan plus its explicit setup; it does not infer, repair, or rerun rotation mechanics.",
    ),
    ServiceDescriptor(

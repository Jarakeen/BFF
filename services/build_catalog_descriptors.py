...
BUILD_SERVICE_DESCRIPTORS: tuple[ServiceDescriptor, ...] = (
    ...
    ServiceDescriptor(
        service_id="build.comp_maker.persistence",
        domain="build",
        purpose="Promote accepted Comp Maker chair plans into canonical player-owned build records before Raid Plan persistence.",
        implementation_path="services.comp_build_persistence_service",
        inputs=("CompPlanState", "CanonicalBuildCatalog"),
        outputs=("CompBuildPersistenceResult", "RaidPlanBuildAssignment"),
        dependencies=("build.catalog.persistence",),
        responsibilities=("comp_maker_assignment_validation", "comp_maker_identity_persistence"),
        behavior=ServiceBehavior.DETERMINISTIC,
        ui_safe=True,
        evidence_class=EvidenceClass.NONE,
        notes="Validates Comp chair identity and preserves explicit Saved Build references. Planned chair configuration remains Raid Plan-owned; no new build_kind=comp records are created. Legacy comp records remain readable for recovery compatibility.",
    ),
    ServiceDescriptor(
        service_id="build.rotation_artifact.persistence",
        domain="build",
        purpose="Persist and restore the latest explicitly completed RotationPlan for a canonical saved build without mutating ESO reference data or regenerating rotation evidence.",
        implementation_path="services.build_rotation_artifact_service",
        inputs=("CanonicalBuildId", "CompletedRotationPlan", "RotationGenerationSetup"),
        outputs=("BuildRotationArtifact", "RotationPlan"),
        dependencies=("build.catalog.persistence",),
        responsibilities=("build_owned_rotation_artifact_persistence",),
        behavior=ServiceBehavior.DETERMINISTIC,
        ui_safe=True,
        evidence_class=EvidenceClass.MIXED,
        notes="Artifacts are keyed by canonical build_id in build_rotations.json. Saving copies only an already-rendered completed plan plus its explicit setup. Loading restores the exact canonical RotationPlan contract and fails closed on malformed saved actions; it does not infer, repair, or rerun rotation mechanics.",
    ),
    ServiceDescriptor(

...
"""Canonical closed Extreme Spell Damage record."""
...
SPELL_DAMAGE_RECORD_VALUE = 13_834.610
SPELL_DAMAGE_WINNER = ("Armor of Truth 5pc", "Kvatch Gladiator 5pc")


@dataclass(frozen=True)
class ExtremeSpellDamageConditionalSnapshot:
    nongear_preclass_spell_damage: float = 5_297.006
    armor_of_truth_spell_damage: float = 589.0
    kvatch_gladiator_spell_damage: float = 1_604.0
    expert_mage_spell_damage: float = 648.0
    higher_max_resource: float = 49_961.0
    font_plus_calculated_defense_percent: float = 0.40
    major_sorcery_percent: float = 0.20
    minor_sorcery_percent: float = 0.10

    @property
    def named_gear_spell_damage(self) -> float:
        return self.armor_of_truth_spell_damage + self.kvatch_gladiator_spell_damage

    @property
    def pre_percent_reference(self) -> float:
        return (
            self.nongear_preclass_spell_damage
            + self.named_gear_spell_damage
            + self.expert_mage_spell_damage
        )

    @property
    def spell_damage(self) -> float:
        return self.pre_percent_reference * (
            1.0
            + self.font_plus_calculated_defense_percent
            + self.major_sorcery_percent
            + self.minor_sorcery_percent
        )
...
class ExtremeSpellDamageRecordService:
    ...
    RUNTIME_PREREQUISITES = (
        ...
        "Spell Power potion supplies Major Sorcery.",
        "External Minor Sorcery is active.",
    ...
    SELF_PROVIDED_CONDITIONS = (
        "Spell Power potion: Major Sorcery",
    ...
    def record(self) -> ExtremeRecordResult:
        ...
        value = snapshot.spell_damage
        if abs(value - SPELL_DAMAGE_RECORD_VALUE) > 0.001:
            raise RuntimeError(
                "Canonical Spell Damage snapshot drifted from the reviewed closed record: "
                f"{value:.3f} != {SPELL_DAMAGE_RECORD_VALUE:.3f}"
            )

        coverage = ExtremeRecordSearchCoverage(
            searched=(
                "reviewed U50 Spell Damage named-gear semantic breakpoint denominator",
                ...
                "potion-active Major Sorcery baseline",
                "external Minor Sorcery named-buff layer",
        ...
        winning_build = {
            "sets": SPELL_DAMAGE_WINNER,
            "nongear_preclass_spell_damage": snapshot.nongear_preclass_spell_damage,
            "named_gear_spell_damage": snapshot.named_gear_spell_damage,
            "expert_mage_spell_damage": snapshot.expert_mage_spell_damage,
            ...
            "major_sorcery_percent": snapshot.major_sorcery_percent,
            "minor_sorcery_percent": snapshot.minor_sorcery_percent,
        ...
        return ExtremeRecordResult.for_objective(
            ...
            explanation=(
                "The contextual potion-active U50 maximum is 13,834.610 Spell Damage.",
                "Armor of Truth 5pc + Kvatch Gladiator 5pc is the sole physical witness tied at the validated maximum.",
                "The denominator replay scored 79 clean physical witnesses and found zero rows above the winner.",
                "The record is conditional because execute, target-state, external Minor Sorcery, potion, and class-runtime prerequisites must coexist.",
            ),
        )


__all__ = [
    "ExtremeSpellDamageConditionalSnapshot",
    "ExtremeSpellDamageRecordService",
    "SPELL_DAMAGE_RECORD_VALUE",
    "SPELL_DAMAGE_WINNER",
]

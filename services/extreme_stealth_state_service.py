...
class ExtremeStealthStateService:
    ...
    exactly once at the formula boundary. Extreme also clamps the final physical
    radius at zero so over-complete reduction packages cannot manufacture a
    negative detection distance.
    """
    ...
    def evaluate_detection_radius(
        ...
        raw_radius = calculate_sneak_detect_range(
        ...
        reduction = min(base, max(0.0, base - radius))
        return ExtremeStealthDetectionResult(
            detection_radius_meters=radius,

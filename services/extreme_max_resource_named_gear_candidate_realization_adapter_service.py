...
"""Adapt the composed Max Resource named-gear frontier to the legacy contract."""
...
class ExtremeMaxResourceNamedGearCandidateRealizationAdapterService:
    """Expose a composed max-resource frontier through the existing gear contract."""
    ...
    def build(
        cls,
        *,
        search: ExtremeMaxResourceNamedGearCandidateSearchResult,
        ...
        if objective not in cls.SUPPORTED_OBJECTIVES:
            raise ValueError(f"unsupported composed max-resource objective {objective!r}")
        if str(search.objective_key or "").strip().casefold() != objective:
            raise ValueError("candidate search objective does not match relevance objective")
...
__all__ = ["ExtremeMaxResourceNamedGearCandidateRealizationAdapterService"]

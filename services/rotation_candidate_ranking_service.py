...
class RotationCandidateRankingService:
    ...
    def _reasons(scorecard: RotationCandidateScorecard) -> tuple[str, ...]:
        ...
        if slot_violations:
            reasons.append(f"{len(slot_violations)} action slot violation(s)")
            for violation in slot_violations:
                requirement = violation.requirement
                scheduled = violation.scheduled_bar or "unspecified"
                allowed = ", ".join(requirement.allowed_bars)
                reasons.append(
                    f"slot legality for {requirement.action_name!r} at "
                    f"{violation.time_seconds:g}s: scheduled {scheduled} bar, "
                    f"slotted on {allowed}; {violation.reason}"
                )
        active_bar_violations = tuple(getattr(scorecard, "active_bar_violations", ()))
        if active_bar_violations:
            reasons.append(f"{len(active_bar_violations)} active-bar violation(s)")
            for violation in active_bar_violations:
                scheduled = violation.scheduled_bar or "unspecified"
                reasons.append(

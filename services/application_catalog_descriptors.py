...
APPLICATION_SERVICE_DESCRIPTORS: tuple[ServiceDescriptor, ...] = (
    ...
    ServiceDescriptor(
        ...
        domain="logs",
        purpose="Aggregate bounded top individual ESO Logs character rankings into role-specific observed gear-set popularity and local snapshot-to-snapshot momentum summaries.",
        implementation_path="services.esologs_trending_service",
        inputs=("EsoLogsClient", "EncounterId", "RankedCharacterSample", "TrendingSnapshotHistory"),
        outputs=("EsoLogsTrendingReport", "RoleTrendingSummary", "TrendingItem", "TrendMovementItem"),
        ...
        encounter_aware=True,
        evidence_class=EvidenceClass.OBSERVATIONAL,
        provenance=("ESO Logs characterRankings", "ESO Logs playerDetails", "local bounded trend snapshots"),
        notes="Popularity and momentum are descriptive observational evidence only. Set counts are per matched top-ranked individual player, never per full team or equipped item. Ranked identities that cannot be matched exactly back to playerDetails are skipped rather than guessed. Making Waves, Cooling Off, New Arrival, and Breakout compare the current sample only with prior locally saved snapshots for the same encounter and role; they are not canonical best-in-slot or game-mechanic claims.",
    ),
    ServiceDescriptor(

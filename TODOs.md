...
-[X] Comp Maker roster-first boundary: Comp Maker owns 4/12-player roster/chair construction, player uniqueness, role/class/build fit, Recruit/open-seat handling, and locked choices; it no longer optimizes raid-wide buff/debuff coverage during roster fill.
-[ ] Optimizer: add the class/race/build/provider coverage optimization that improves an assembled roster for unique raid buffs/debuffs, reports duplicates/uncovered effects/provider provenance, and preserves hard player/role/assignment constraints rather than rebuilding the roster from scratch.

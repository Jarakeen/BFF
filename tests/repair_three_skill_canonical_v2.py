...
REPORT_PATH = BASE_DIR / "data" / "processed" / "three_skill_canonical_repair_v2_report.json"
...
def main():
    ...
    print(" Three Skill Canonical Repair v2")
    print("=" * 60)
    print()
    print("This version reads ESO-Hub directly.")
    print("It does NOT read the crosswalk.")
    print()
    ...
    try:
        for skill_id, name, skill_line, entity_id in REPAIRS:
            ...
            if not url:
                errors.append(f"{name}: ESO-Hub URL missing")
                continue
        ...
        report = {
            "summary": {
                ...
                "entities_created": sum(
                    x["entity_action"] == "created"
                    for x in actions
                ),
                "mappings_inserted": sum(
                    x["mapping_action"] == "inserted"
                    for x in actions
                ),
                "errors": 0,
            },

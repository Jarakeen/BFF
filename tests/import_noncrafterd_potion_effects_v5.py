...
def main():
    ...
    print(" Non-Crafted Potion Effect Relationship Importer v5")
    print("=" * 60)
    print()
    ...
    try:
        ...
        for potion in potions:
            ...
            for item in potion.get("effects", []):
                ...
                if len(variants) > 1:
                    # The normalized source can contain both the underlying
                    # effect entity (for example "Brutality") and its ranked
                    # canonical buff entity ("Major Brutality"). Both resolve
                    # to the same raw effect, so the unranked relationship
                    # must use the rank carried by its ranked counterpart in
                    # this same potion.
                    ...
                    if canonical_name.startswith("Major "):
                        ...
                        for other_item in potion.get("effects", []):
                            other_name = other_item.get("name") or ""
                            if other_name == f"Major {canonical_name}":
                                ranked_counterparts.append("Major")
                            elif other_name == f"Minor {canonical_name}":
                                ranked_counterparts.append("Minor")

                        ranked_counterparts = sorted(set(ranked_counterparts))

                        if len(ranked_counterparts) == 1:
                            desired_variant_type = ranked_counterparts[0]
                        elif len(ranked_counterparts) == 0:
                            errors.append(
                                f"{potion_name}: multiple existing Potions "
                                f"variants for {effect_name!r}; canonical "
                                f"buff name {canonical_name!r} has no ranked "
                                f"counterpart in this potion"
                            )
                            continue
                        else:
                            errors.append(
                                f"{potion_name}: canonical buff name "
                                f"{canonical_name!r} has multiple ranked "
                                f"counterparts: "
                                f"{', '.join(ranked_counterparts)}"
                            )
                            continue
        ...
        if errors:
            ...
            raise RuntimeError(
                "Non-crafted potion effect import v5 failed."
            )

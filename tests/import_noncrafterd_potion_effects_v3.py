...
def main():
    ...
    print(" Non-Crafted Potion Effect Relationship Importer v3")
    print("=" * 60)
    print()
    ...
    try:
        ...
        for potion in potions:
            ...
            for item in potion.get("effects", []):
                ...
                if len(variants) > 1:
                    # Prefer a variant whose existing source name contains
                    # the current potion name. Otherwise do not guess.
                    matching = []
                    ...
                    for variant in variants:
                        ...
                        for source_id, source_name in rows:
                            if potion_name.casefold() in (
                                source_name or ""
                            ).casefold():
                                matching.append(
                                    (variant, source_id, source_name)
                                )

                    if len(matching) == 1:
                        variant = matching[0][0]
                    elif len(matching) == 0:
                        errors.append(
                            f"{potion_name}: multiple existing Potions "
                            f"variants for {effect_name!r}; "
                            f"none identifies this potion"
                        )
                        continue
                    else:
                        errors.append(
                            f"{potion_name}: multiple existing Potions "
                            f"variants identify this potion"
                        )
                        continue
                else:
                    variant = variants[0]
        ...
        if errors:
            ...
            raise RuntimeError(
                "Non-crafted potion effect import v3 failed."
            )

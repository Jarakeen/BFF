...
- **Finch shared import** exposes explicit **Get Shared Teams** and **Get Shared Plans** actions. Team import brings in Team identity plus schedule/time zone/current focus only and never creates Personnel; Raid Plan import brings in the shared plan outline only and never creates Personnel, Characters, saved builds, or local build identity. A local plan with the same stable plan id is replaced only after an explicit confirmation.

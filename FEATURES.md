...
- **Finch shared publishing** exposes explicit nonblocking Publish Team and Publish Raid Plan actions. Shared Team schema v1 contains Team name, saved schedule/current focus, and basic active roster identity; Shared Raid Plan schema v1 contains stable plan identity, trial/difficulty/status, Team name, and seat/player/character/class/role outline. Personnel notes, Discord URLs, local database ids, full build payloads, plan notes, and chair notes remain local by design.
- **Finch shared retrieval / Copy to Local** exposes explicit **Get Shared Teams** and **Get Shared Plans** actions. Finch snapshots remain read-only previews until the user explicitly copies one locally. A shared Team copy creates a new non-colliding local Team containing schedule/time zone/current focus only and never creates Personnel; a shared Raid Plan copy creates a new non-colliding local Raid Plan outline and never creates Personnel, Characters, saved builds, or local build identity. Shared snapshots never overwrite matching local Team names or Raid Plan ids.

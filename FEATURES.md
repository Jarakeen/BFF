...
- **Comp Builder roster-first planning** keeps imported/selected groups at their real ESO size (4 or 12), treats `Recruit` as an open prescription slot, scopes saved-build choices to the loaded player, and preserves known player/character identity when a build decision is still unresolved; the Option 2 workspace keeps Team and Team Health primary, uses a compact selected-player/build recommendation panel, and collapses advanced assignment fields by default

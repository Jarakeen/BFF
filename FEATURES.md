...
- Phase 14 Comp Maker is **roster-first**: bulk Fill from Roster / Auto-Fill operates on canonical `CompPlanState` to fill unresolved chair/build decisions while preserving explicit/locked choices, consuming each real saved player at most once, preferring canonical player/character/build identity, and allowing reusable reference evidence for Recruit chairs without inventing ownership. Raid-wide buff/debuff optimization is intentionally downstream in Optimizer; Team Health remains informational feedback about the roster being assembled.

...
- Assignments separates buff/debuff ownership from utility/mechanic jobs, combines primary/secondary support responsibilities into one visible column, moves notes into Selected Spot, and shows Plan Snapshot instead of a Mechanic Coverage link card
...
- Phase 14 **Auto-Fill Builds** operates on canonical `CompPlanState`: explicit/locked or already-planned build and gear decisions are preserved, explicit primary provider assignments become chair-local constraints when canonically mapped, general missing support remains a raid-wide objective, and open Recruit chairs may receive planning evidence without silently becoming the owner of a referenced saved build.
- **Coverage planning semantics:** Raid Plan Coverage answers whether the planned group has a source for each tracked buff/debuff. Planned gear, conditional/proc sources, and explicit provider assignments count as **Covered** for planning; evidence labels separately describe Supported, Conditional, Planned/runtime-unproven, duplicate ownership, or Missing. Rotation and Raid Review remain responsible for execution and observed uptime.
- Phase 14 Comp Maker Team Health now reads the canonical `CompPlanState` for Raid Plan-bound sessions rather than reconstructing coverage from presentation tables. Planned gear uses the same reviewed Conditional coverage evaluator as Coverage, including perfected-set family normalization such as `Perfected Roaring Opportunist` resolving through the reviewed `Roaring Opportunist` provider relationship; explicit primary/backup assignments remain raid-lead intent and are checked separately against planned provider evidence, so assigned-but-unproven and duplicate-primary states remain visible. Duplicate effects, missing required effects, open player seats, and open gear decisions are derived from the state that Save will persist.

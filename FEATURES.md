...
- **Generated sustained-DPS runtime evaluation** — evaluates one explicit hypothetical DD build + caller-owned progression + legal dual-bar gear state + RotationPlan + authoritative runtime history + explicit target through the existing Phase 14 Combat Simulation stack. Bar-legal named gear buffs affect exact runtime build contexts; active non-named timed gear effects currently fail closed pending the generic timed-effect context bridge.

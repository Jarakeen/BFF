...
- **Global Objective #32 proof runner** — runs the structural global tree and derives structural race/class-route/attribute coverage from the exact same validated family adapter that created those branches. Remaining canonical-axis proofs still require explicit denominator-scope equivalence, and the generated-axis inventory must confirm every canonical mutation axis is physically present in the searched tree before the theoretical flag can close.

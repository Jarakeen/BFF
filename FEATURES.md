- **Finch Discord Raid Companion** — Discord slash commands project saved FoundryDock Raid Plans, player build/assignment briefs, reviewed encounter strategies, saved raid maps, and Team schedules without creating a second source of raid truth. Finch also sends configurable recurring raid reminders from the Team schedule already stored in FoundryDock; Discord tokens remain environment-only and Personnel Discord names are optional matching aliases.
- **Finch registered-player experience** — Discord users can register once against an existing FoundryDock Team + Personnel identity, then use private `/me`, `/build`, and `/schedule` views without repeatedly typing player names. Registration stores only a Discord-to-Personnel binding; FoundryDock remains authoritative. Player-facing responses use compact branded Discord embeds rather than raw plan dumps.

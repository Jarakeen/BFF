...
- **End-to-end Objective #32 proof result** — runs the generated pipeline search, composes caller-supplied canonical axis proofs, automatically adds proven local runtime-state coverage when that frontier is searched, and then applies the theoretical closure gate. The returned result exposes the best modeled DPS, finite-denominator maximum proof, and theoretical maximum proof separately rather than letting one optimistic boolean do three jobs.

...
- **Finch Collaboration overview** adds one read-only Team Operations page that lists shared Teams, Raid Plans, Readiness, and Coverage together with publisher, UTC update time, provenance/status, summary counts, and an **Open Workspace** handoff to the owning local page. A compact attention summary and filters surface **Changed Since Copy**, **Not Copied**, **Readiness Gaps**, and **Coverage Gaps** from structured state rather than display-text parsing. The overview never publishes, imports, or mutates local state; refresh remains an explicit user action.

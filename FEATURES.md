...
- **Delayed Ultimate timing frontier** — Objective #32 can now enumerate every legal delayed Ultimate cast/skip sequence across the exact same-bar skill slots of one seed rotation while consuming a shared explicit Ultimate resource timeline. Same-timestamp generation is never borrowed ahead of a cast. This proves the finite delayed-timing family for a seed plan without yet claiming that the global rotation-policy axis has integrated it.

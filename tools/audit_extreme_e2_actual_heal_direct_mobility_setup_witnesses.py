...
"""Read-only route audit for H1 direct-mobility setup witnesses used by Crusader."""
...
def main() -> int:
    ...
    for route in routes:
        ...
        witness = setup.witness(DEALS_DIRECT_MOBILITY_DAMAGE, context)
    ...
    print("EXTREME E2 H1 DIRECT-MOBILITY SETUP-WITNESS AUDIT")
    print(f"database={database}")
    print(f"route_count={len(routes)}")
    print(f"routes_with_direct_mobility_witness={proven}")
    print(f"routes_without_direct_mobility_witness={len(missing)}")
    print("DISTINCT DIRECT-MOBILITY WITNESSES")
    ...
    if missing:
        print("ROUTES WITHOUT DIRECT-MOBILITY WITNESS")
    ...
    print(
        "NEXT_STEP=admit Crusader only if every relevant H1 route has a route-legal "
        "non-Ultimate direct-damage Blink/Charge/Leap/Teleport/Pull witness that can "
        "be materialized on the inactive bar"
    )
    return 0

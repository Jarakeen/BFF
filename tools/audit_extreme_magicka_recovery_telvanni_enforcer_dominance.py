...
"""Exact physical dominance check for Telvanni Enforcer against the legal Willow witness.

The broad flat screen intentionally overcounts overlapping mapped/description evidence.
For Telvanni Enforcer that produces a proof-safe 738 Recovery ceiling even though the
canonical state split owns one +369 Magicka Recovery branch while Bracing. This audit
keeps the generic ceiling as a control, then scores the exact mapped +369 branch in a
physical seven-Light realization.
"""
...
def main() -> int:
    ...
    filtered = ExtremeArmorWeightFilteredSlotEligibilityService.build(database, raw_eligibility, required_armor_weight="Light")
    ...
    if len(target_rows) != 1:
        ...
        if target_evidence is None:
            unresolved.append("Telvanni Enforcer canonical Recovery evidence missing")
    ...
    if target is not None and target_evidence is not None:
        ...
        generic_ceiling = float(upper.total_special_ceiling)
        exact_special = float(mapped_positive_recovery_flat(target_evidence))
        if exact_special <= 0.0:
            unresolved.append("Telvanni Enforcer exact mapped Magicka Recovery branch missing")
        ...
        if constrained is None or not constrained.winner_found:
            unresolved.append("no physically realizable seven-Light Telvanni Enforcer witness")
        else:
    ...
    if len(willow_rows) != 1:
        ...
        if willow_search is None or not willow_search.winner_found:
            unresolved.append("no physical Willow's Path witness")
        else:
    ...
    shared_flat = float(BASE_MAGICKA_RECOVERY) + three_infused + drink + atronach_delta + cp_total + race_flat
    ...
    target_final_upper = final_upper(shared_flat=shared_flat, structural=target_structural, special=exact_special)
    generic_final_control = final_upper(shared_flat=shared_flat, structural=target_structural, special=generic_ceiling)
    margin = willow_final - target_final_upper
    dominated = target_realization is not None and margin > 1e-9
    unique_unresolved = tuple(dict.fromkeys(unresolved))
    closed = bool(not unique_unresolved and willow_realization is not None and dominated)

    print("EXTREME MAGICKA RECOVERY TELVANNI ENFORCER DOMINANCE")
    print(f"database={database}")
    print(f"shared_flat_floor={shared_flat:.3f}")
    print(f"willow_constructive_final={willow_final:.3f}")
    print(f"telvanni_generic_special_ceiling={generic_ceiling:.3f}")
    print(f"telvanni_exact_special_flat_ceiling={exact_special:.3f}")
    print(f"telvanni_physical_structural_upper={target_structural:.3f}")
    print(f"telvanni_generic_final_control={generic_final_control:.3f}")
    print(f"telvanni_optimistic_prepercent={target_structural + exact_special:.3f}")
    print(f"telvanni_final_upper={target_final_upper:.3f}")
    print(f"margin_to_willow={margin:.3f}")
    print(f"dominated={dominated}")
    if target_realization is not None:
        print(f"telvanni_witness_sets={tuple(zip(target_realization.set_names, target_realization.counts))!r}")
    ...
    print(f"telvanni_physical_witness={target_realization is not None}")
    ...
    print(f"telvanni_branch_closed={closed}")
    print("NEXT_STEP=remove Telvanni Enforcer from the Willow-rebased flat survivor queue" if closed else "NEXT_STEP=close only the reported Telvanni Enforcer blockers")

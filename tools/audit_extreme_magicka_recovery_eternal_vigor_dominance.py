...
def main() -> int:
    ...
    if target is not None and target_evidence is not None:
        ...
        if constrained is None or not constrained.winner_found:
            unresolved.append("no physically realizable seven-Light Eternal Vigor witness")
        else:
    ...
    print("EXTREME MAGICKA RECOVERY ETERNAL VIGOR DOMINANCE")
    ...
    if target_realization is not None:
        print(f"eternal_vigor_witness_sets={tuple(zip(target_realization.set_names, target_realization.counts))!r}")
    ...
    print(f"eternal_vigor_physical_witness={target_realization is not None}")
    ...
    print(f"eternal_vigor_branch_closed={closed}")
    print("NEXT_STEP=remove Eternal Vigor from the Willow-rebased flat survivor queue" if closed else "NEXT_STEP=close only the reported Eternal Vigor blockers")

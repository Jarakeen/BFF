...
"""Construct a proof-owned Max Stamina component ledger without exhaustive search."""
...
TARGET_STAT = StatId.MAX_STAMINA
...
def main() -> int:
    ...
    provisioning = ExtremeResourceProvisioningProjectionService(provisioning_repository).build(OBJECTIVE)
    ...
    special = ExtremeMaxResourceSpecialNamedGearBranchService(relevance).build(special_pairs)

    ranked_gear = tuple(
        sorted(
            (row for row in relevance.relevant if float(row.reviewed_delta) > 0.0),
            key=lambda row: (
                -float(row.reviewed_delta), int(row.piece_count),
                row.set_name.casefold(), row.set_name,
            ),
        )
    )
    ...
    attribute_delta = (
        ...
        if attributes.projection_complete else 0.0
    )
    ...
    print("EXTREME MAX STAMINA CONSTRUCTIVE COMPONENT AUDIT")
    ...
    print(f"race={race_name or '<unresolved>'} delta={race_delta:g} projection_complete={race.projection_complete}")
    if route_witness is not None:
        print(
            f"class_routes_source={route.source_route_count} projected_signatures={route.projected_route_count} "
            f"dominant_signature={route.signature!r} witness={route_witness.base_class.value} "
            f"subclassed={route_witness.is_subclassed} lines={tuple(route_witness.equipped_skill_lines)!r} "
            f"projection_complete={route.projection_complete}"
        )
    else:
        print(f"class_routes_source={route.source_route_count} projection_complete=False")
    print(f"attributes={attributes.target_points if attributes.projection_complete else 0} points delta={attribute_delta:g} projection_complete={attributes.projection_complete}")
    print(f"mundus={mundus.witness or '<unresolved>'} base_delta={mundus_delta:g} projection_complete={mundus.projection_complete}")
    ...
    print(f"potion_formulas_reviewed={potion.formulas_reviewed} relevant_formulas={len(potion.relevant_formulas)} objective_irrelevance_proven={potion.objective_irrelevance_proven}")
    print(f"armor_best_direct_glyph_delta={armor_delta:g} divines={best_armor.divines_count if best_armor else 0} infused={best_armor.infused_count if best_armor else 0} frontier_states={len(armor.states)} denominator_proven={armor.denominator_proven}")
    print(f"jewelry_static_delta={jewelry_delta:g} denominator_proven={jewelry.denominator_proven}")
    if best_jewelry is not None:
        print(f"jewelry_traits={best_jewelry.traits!r}")
    print(f"champion_points_delta={cp_delta:g} denominator_proven={champion.denominator_proven}")
    print()

    print("RANKED POSITIVE MAX-STAMINA GEAR BREAKPOINTS")
    print(f"gear_breakpoints_reviewed={len(relevance.evidence)} relevance_denominator_proven={relevance.denominator_proven}")
    for row in ranked_gear[: max(0, int(args.gear_limit))]:
        print(f"  {row.reviewed_delta:10.3f}  {row.piece_count:2d}pc  {row.set_name}")
    ...
    for branch in special.branches:
        ...
        print(
            ...
            f"condition={getattr(branch, 'condition', None) or '<none>'} "
            f"rule={getattr(rule, 'value', rule) or '<none>'}"
        )

    unresolved = tuple(dict.fromkeys(
        str(item)
        for item in (
            *race.unresolved, *route.unresolved, *attributes.unresolved,
            *mundus.unresolved, *provisioning.unresolved, *potion.unresolved,
            *armor.unresolved, *jewelry.unresolved, *champion.unresolved,
            *relevance.unresolved, *special.unresolved,
        )
        if str(item)
    ))
    print()
    ...
    print("NEXT_STEP=canonical-score the strongest legal Max Stamina ordinary witness, then use it as the threshold for targeted special/runtime closure")

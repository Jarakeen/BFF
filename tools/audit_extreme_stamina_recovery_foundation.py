...
"""Audit the reusable Extreme foundation for the Stamina Recovery record.

This is a denominator/discovery audit, not a final record claim. It deliberately
reuses shared Recovery services already proven during Magicka Recovery and prints
remaining Stamina-specific blockers explicitly instead of assuming symmetry.
"""
...
def _positive_passive_branches(database: Path):
    rows = []
    unresolved = []
    for passive in ExtremeSkillUniverseService(database).all_player_skills():
        if not passive.is_passive:
            continue
        if not ExtremeRecoveryPassiveSpecialBranchService.mentions_objective_recovery(
            passive.description,
            OBJECTIVE,
        ):
            continue
        ...
        if branch is None:
            ...
            continue
        if not branch.can_raise_self:
            continue
        rows.append(branch)
    rows.sort(
        key=lambda row: (
            str(row.passive.domain.value),
            row.passive.skill_line.casefold(),
            row.passive.name.casefold(),
        )
    )
...
def main() -> int:
    ...
    passives, passive_unresolved = _positive_passive_branches(database)
    coverage = ExtremeObjectiveCoverageService.coverage_for(OBJECTIVE)

    print("EXTREME STAMINA RECOVERY FOUNDATION AUDIT")
    print(f"database={database}")
    print(f"objective={OBJECTIVE}")
    print(f"base_stamina_recovery={BASE_STAMINA_RECOVERY:.3f}")
    print()
    ...
    print("POSITIVE PASSIVE DENOMINATOR")
    print(f"positive_passive_branch_count={len(passives)}")
    for branch in passives:
        print(
            "  passive "
            f"domain={branch.passive.domain.value!r} line={branch.passive.skill_line!r} "
            f"name={branch.passive.name!r} kind={branch.kind.value!r} "
            f"flat_ceiling={branch.flat_ceiling!r} percent_ceiling={branch.percent_ceiling!r} "
            f"condition={branch.condition!r}"
        )
    print(f"passive_unresolved_count={len(passive_unresolved)}")
    for item in passive_unresolved:
    ...
    foundation_numeric_axes_available = all(
        (
            ...
            not passive_unresolved,
        )
    )
    print(f"foundation_numeric_axes_available={foundation_numeric_axes_available}")
    print("stamina_recovery_record_closed=False")
    print(
        "NEXT_STEP=close Stamina-specific class/bar, CP, jewelry opportunity cost, "
        "named gear, named buffs, and contextual Recovery states using shared Recovery proofs"
    )
    return 0

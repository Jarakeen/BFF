...
"""Exact physical dominance check for The Worm's Raiment against the legal Willow witness.

The Worm's Raiment survives the Willow-rebased capacity screen by a modest margin.
This audit requires The Worm's Raiment 5pc physically, externalizes its Recovery
mechanic, finds the best legal seven-Light ordinary companion package, then adds the
full proof-safe flat ceiling. If that final upper bound is below the constructive
Willow's Path lower bound, The Worm's Raiment is globally dominated and its runtime
condition no longer matters.
"""
...
TARGET_NAME = "The Worm's Raiment"
TARGET_PIECES = 5
...
def main() -> int:
    ...
    if len(target_rows) != 1:
        ...
        if target_evidence is None:
            unresolved.append("The Worm's Raiment canonical Recovery evidence missing")
    ...
    if target is not None and target_evidence is not None:
        ...
        if constrained is None or not constrained.winner_found:
            unresolved.append("no physically realizable seven-Light The Worm's Raiment witness")
        else:
    ...
    print("EXTREME MAGICKA RECOVERY THE WORM'S RAIMENT DOMINANCE")
    print(f"database={database}")
    print(f"shared_flat_floor={shared_flat:.3f}")
    print(f"willow_constructive_final={willow_final:.3f}")
    print(f"worm_special_flat_ceiling={special_ceiling:.3f}")
    print(f"worm_physical_structural_upper={target_structural:.3f}")
    print(f"worm_optimistic_prepercent={target_structural + special_ceiling:.3f}")
    print(f"worm_final_upper={target_final_upper:.3f}")
    print(f"margin_to_willow={margin:.3f}")
    print(f"dominated={dominated}")
    if target_realization is not None:
        print(f"worm_witness_sets={tuple(zip(target_realization.set_names, target_realization.counts))!r}")
    ...
    print(f"worm_physical_witness={target_realization is not None}")
    ...
    print(f"worm_branch_closed={closed}")
    print("NEXT_STEP=remove The Worm's Raiment from the Willow-rebased flat survivor queue" if closed else "NEXT_STEP=close only the reported The Worm's Raiment blockers")

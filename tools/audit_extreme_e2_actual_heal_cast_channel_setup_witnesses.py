...
"""Read-only route audit for H1 cast/channel setup witnesses used by Soulshine."""
...
def main() -> int:
    ...
    for route in routes:
        ...
        witness = setup.witness(HAS_CAST_OR_CHANNEL_TIME, context)
    ...
    print("EXTREME E2 H1 CAST/CHANNEL SETUP-WITNESS AUDIT")
    print(f"database={database}")
    print(f"route_count={len(routes)}")
    print(f"routes_with_cast_or_channel_witness={proven}")
    print(f"routes_without_cast_or_channel_witness={len(missing)}")
    print("DISTINCT CAST/CHANNEL WITNESSES")
    ...
    if missing:
        print("ROUTES WITHOUT CAST/CHANNEL WITNESS")
    ...
    print(
        "NEXT_STEP=admit Soulshine only if every relevant H1 route has a route-legal "
        "non-Ultimate cast/channel witness that can be materialized on the inactive bar"
    )
    return 0

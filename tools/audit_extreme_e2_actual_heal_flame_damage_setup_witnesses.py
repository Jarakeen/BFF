...
"""Read-only route audit for H1 Flame-damage setup witnesses used by Burning Spellweave."""
...
def main() -> int:
    ...
    for route in routes:
        ...
        witness = setup.witness(DEALS_FLAME_DAMAGE, context)
    ...
    print("EXTREME E2 H1 FLAME-DAMAGE SETUP-WITNESS AUDIT")
    print(f"database={database}")
    print(f"route_count={len(routes)}")
    print(f"routes_with_flame_damage_witness={proven}")
    print(f"routes_without_flame_damage_witness={len(missing)}")
    print("DISTINCT FLAME-DAMAGE WITNESSES")
    ...
    if missing:
        print("ROUTES WITHOUT FLAME-DAMAGE WITNESS")
    ...
    print(
        "NEXT_STEP=admit Burning Spellweave only if every relevant H1 route has a "
        "route-legal non-Ultimate Flame-damage witness that can be materialized on "
        "the inactive bar"
    )
    return 0

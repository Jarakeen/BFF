...
"""Exact physical dominance check for Prowler's Talisman against the legal Willow witness.

Prowler's Talisman mixes direct Recovery with Max Magicka. The shared direct-flat
upper-bound helper already grants both the full direct Recovery ceiling and the full
remaining Enlivening headroom attributable to the same special branch. This audit
requires Prowler's Talisman 1pc physically, finds the best legal seven-Light ordinary
companion package, then compares that proof-safe final upper bound against the legal
Willow's Path lower bound.
"""
...
TARGET_NAME = "Prowler's Talisman"
TARGET_PIECES = 1
...
def main() -> int:
    ...
    if len(target_rows) != 1:
        ...
        if target_evidence is None:
            unresolved.append("Prowler's Talisman canonical Recovery evidence missing")
    ...
    if target is not None and target_evidence is not None:
        ...
        mapped_flat = float(upper.mapped_flat)
        triaged_flat = float(upper.triaged_flat)
        max_magicka_recovery_ceiling = float(upper.max_magicka_recovery_ceiling)
        ...
        if constrained is None or not constrained.winner_found:
            unresolved.append("no physically realizable seven-Light Prowler's Talisman witness")
        else:
    ...
    if len(willow_rows) != 1:
        ...
        if willow_search is None or not willow_search.winner_found:
            ...
            direct, magicka = companion_pair_totals(
                willow_realization, pair_scores, excluded_set_id=willow_id
            )
    ...
    target_final_upper = final_upper(
        shared_flat=shared_flat,
        structural=target_structural,
        special=special_ceiling,
    )
    ...
    print("EXTREME MAGICKA RECOVERY PROWLER'S TALISMAN DOMINANCE")
    print(f"database={database}")
    print(f"shared_flat_floor={shared_flat:.3f}")
    print(f"willow_constructive_final={willow_final:.3f}")
    print(f"prowler_mapped_flat={mapped_flat:.3f}")
    print(f"prowler_triaged_flat_ceiling={triaged_flat:.3f}")
    print(f"prowler_max_magicka_recovery_ceiling={max_magicka_recovery_ceiling:.3f}")
    print(f"prowler_special_flat_ceiling={special_ceiling:.3f}")
    print(f"prowler_physical_structural_upper={target_structural:.3f}")
    print(f"prowler_optimistic_prepercent={target_structural + special_ceiling:.3f}")
    print(f"prowler_final_upper={target_final_upper:.3f}")
    print(f"margin_to_willow={margin:.3f}")
    print(f"dominated={dominated}")
    if target_realization is not None:
        print(f"prowler_witness_sets={tuple(zip(target_realization.set_names, target_realization.counts))!r}")
    ...
    print(f"prowler_physical_witness={target_realization is not None}")
    ...
    print(f"prowler_branch_closed={closed}")
    print("NEXT_STEP=remove Prowler's Talisman from the Willow-rebased flat survivor queue" if closed else "NEXT_STEP=close only the reported Prowler's Talisman blockers")

...
"""Exact physical dominance check for Wrathsun against the legal Willow witness.

Wrathsun survives the Willow-rebased capacity screen. This audit requires Wrathsun
5pc physically, externalizes its stacked Recovery mechanic, finds the best legal
seven-Light ordinary companion package, then adds the full proof-safe stacked flat
ceiling. If that final upper bound is below the constructive Willow's Path lower
bound, Wrathsun is globally dominated and runtime stack feasibility no longer
matters.
"""
...
def main() -> int:
    ...
    if len(target_rows) != 1:
        ...
        if target_evidence is None:
            unresolved.append("Wrathsun canonical Recovery evidence missing")
    ...
    if target is not None and target_evidence is not None:
        ...
        if constrained is None or not constrained.winner_found:
            unresolved.append("no physically realizable seven-Light Wrathsun witness")
        else:
    ...
    print("EXTREME MAGICKA RECOVERY WRATHSUN DOMINANCE")
    print(f"database={database}")
    print(f"shared_flat_floor={shared_flat:.3f}")
    print(f"willow_constructive_final={willow_final:.3f}")
    print(f"wrathsun_special_flat_ceiling={special_ceiling:.3f}")
    print(f"wrathsun_physical_structural_upper={target_structural:.3f}")
    print(f"wrathsun_optimistic_prepercent={target_structural + special_ceiling:.3f}")
    print(f"wrathsun_final_upper={target_final_upper:.3f}")
    print(f"margin_to_willow={margin:.3f}")
    print(f"dominated={dominated}")
    if target_realization is not None:
        print(f"wrathsun_witness_sets={tuple(zip(target_realization.set_names, target_realization.counts))!r}")
    ...
    print(f"wrathsun_physical_witness={target_realization is not None}")
    ...
    print("NEXT_STEP=remove Wrathsun from the Willow-rebased flat survivor queue" if closed else "NEXT_STEP=close only the reported Wrathsun blockers")

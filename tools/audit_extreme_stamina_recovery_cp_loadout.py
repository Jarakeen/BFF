...
"""Audit the legal Champion Point loadout for Extreme Stamina Recovery.
...
Only CP mechanics semantically capable of raising Stamina Recovery can block this
audit; unrelated unresolved stars remain outside this objective denominator.
"""
...
def main() -> int:
    ...
    print("EXTREME STAMINA RECOVERY CHAMPION POINT LOADOUT")
    ...
    print(f"stamina_recovery_cp_loadout_closed={closed}")
    if selected_enlivening is not None and closed:
        print("NEXT_STEP=reuse the same-build Max Magicka witness to score Enlivening exactly for the Stamina Recovery build")
    elif selected_enlivening is not None:
        print("NEXT_STEP=close only the reported Recovery-relevant CP blockers, then score Enlivening on the same build")
    elif closed:
        print("NEXT_STEP=compose the legal CP loadout with the whole-build Stamina Recovery reference")
    else:

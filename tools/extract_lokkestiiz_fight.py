...
OUTPUT_DIR = Path("data/normalized/lokkestiiz")
...
def normalize_fight(
    ...
    abilities = Counter(
        ...
        for event in normalized_events
        if (
            event.get("abilityGameID") is not None
            or event.get("ability") is not None
        )
    )
    ...
    return {
        ...
        "fight": {
            ...
            "encounterID": fight.get(
                "encounterID"
            ),
            "difficulty": fight.get(
                "difficulty"
            ),
            "startTime": start,
            "endTime": end,
...
def print_summary(
    ...
    print(
        f"Fight ID:       {fight.get('id')}"
    )
    print(
        f"Encounter:      {fight.get('name')}"
    )
    print(
        f"Encounter ID:   {fight.get('encounterID')}"
    )
    print(
        f"Difficulty:     {fight.get('difficulty')}"
    )
    print(
        f"Duration:       {fight.get('duration_ms')} ms"
    )
    print(
    ...
    print(
        f"Abilities:      {summary['unique_abilities']}"
    )
    print(
        f"Sources:        {summary['unique_sources']}"
    )
    print(
        f"Targets:        {summary['unique_targets']}"
    )
...
def main() -> None:
    ...
    for raw_file in RAW_FILES:
        ...
        for fight_id, fight in fights:
            ...
            output_path = (
                OUTPUT_DIR
                / f"{report_code}_fight_{fight_id}.json"
            )
    ...
    print("=" * 80)
    print(
        f"Target fights extracted: {found}"
    )
    print("=" * 80)

...
"""Canonically score the complete targeted Max Magicka special/runtime gear frontier.

The ordinary named-gear denominator is already closed against the current legal
108,319 Max Magicka incumbent. This audit therefore does not reopen the broad named
set search. It classifies only the production non-ordinary Max Magicka breakpoints,
enumerates every non-empty special subset that can fit a canonical topology, and uses
the proof-safe semantic special-subset search to find the strongest physically legal
ordinary filler for each fixed subset/topology.

Each resulting witness is then scored through the normal Extreme canonical stack:
...
def main() -> int:
    ...
    for topology in topologies.topologies:
        for size in range(1, len(branches) + 1):
            for subset in combinations(branches, size):
                ...
                if not owner._subset_fits_counts(topology, subset_tuple):
                    continue
                subset_searches += 1
                searched_special_subsets.add(tuple(sorted((int(row.set_id), int(row.piece_count)) for row in subset_tuple)))
    ...
    for row in sorted(branches, key=lambda x: (x.piece_count, x.set_name.casefold())):
        ...
        print(f"  {row.set_name} {row.piece_count}pc kind={row.kind.value} condition={row.condition or '<none>'} rule={rule or '<none>'}")
    for item in classified.unresolved:
        print(f"  classification_unresolved: {item}")
    ...
    best = scored[0]
    print("\nCANONICAL SPECIAL/RUNTIME SCORE")
    ...
    for row in scored[:10]:
        ...
        print(f"    mundus={payload.get('mundus')!r} food={payload.get('food')!r} potion={payload.get('potion')!r}")
    ...
    print("\nBEST SPECIAL/RUNTIME CANDIDATE")
    ...
    if beats:
        ...
        print("NEXT_STEP=special/runtime named gear is closed; 108319 survives the full named-gear denominator")
    else:
        print("NEXT_STEP=resolve the reported special/runtime coverage or canonical warnings before closing the denominator")

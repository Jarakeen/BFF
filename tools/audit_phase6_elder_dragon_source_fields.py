...
TARGET_RANKS = (5578, 5579)


@dataclass(frozen=True)
class ElderDragonSourceRow:
    skill_rank_id: int
    ability_id: int
    name: str
...
def _looks_relevant(text: str) -> bool:
    lower = text.casefold()
    return (
        "current amount" in lower
        or "elder dragon" in lower
        or "$1" in lower
        or "health recovery" in lower
        or "physical resistance" in lower
        or "spell resistance" in lower
        or "resistance" in lower
        or "health" in lower
    )


def load_elder_dragon_source_rows(
    database_path: str | Path,
    *,
    skill_rank_ids: tuple[int, ...] = TARGET_RANKS,
) -> tuple[ElderDragonSourceRow, ...]:
    ...
    results: list[ElderDragonSourceRow] = []
    for row in rows:
        populated: list[str] = []
        matching: list[str] = []
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Audit real source fields for the two unresolved Elder Dragon Current amount rows."
    )
    parser.add_argument("--database", default=str(DEFAULT_DATABASE))
    args = parser.parse_args()

    rows = load_elder_dragon_source_rows(args.database)
    print("\n========================================")
    print(" PHASE 6 ELDER DRAGON SOURCE FIELDS")
    print("========================================")

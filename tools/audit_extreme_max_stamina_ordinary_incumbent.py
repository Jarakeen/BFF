...
"""Find a clean Max Stamina incumbent from the exact ordinary named-gear frontier.
...
at the global best unconditional flat Max Stamina delta, then canonical-scores those
witnesses across the proof-reduced dominant class route and complete resource armor
weight/trait/glyph frontier.

Mundus, provisioning, potion, jewelry, Champion Points, active-bar witnesses,
runtime resource passives, and the six-Home-Keep Emperor snapshot remain owned by
...
def main() -> int:
    ...
    if not race_projection.projection_complete:
        raise RuntimeError(
            "Max Stamina race projection is incomplete: " + "; ".join(race_projection.unresolved)
        )
    if not attribute_projection.projection_complete:
        raise RuntimeError(
            "Max Stamina attribute projection is incomplete: "
    ...
    if not ordinary_rows or global_ordinary_delta == float("-inf"):
        raise RuntimeError("Exact ordinary Max Stamina search returned no physical winner")
    ...
    if not jewelry_catalog.denominator_proven or jewelry_state is None:
        raise RuntimeError(
            "Max Stamina jewelry denominator is incomplete: " + "; ".join(jewelry_catalog.unresolved)
        )
    ...
    scored: list[tuple[float, tuple[object, ...], object, object, str, dict, tuple[str, ...]]] = []
    score_calls = 0
    unresolved_calls = 0
    ...
    best = scored[0]
    best_value, _identity, best_gear, best_armor, best_bar, best_payload, best_unresolved = best

    print("EXTREME MAX STAMINA ORDINARY INCUMBENT")
    print(f"database={database}")
    print("mode=exact_best_ordinary_gear+dominant_route+complete_resource_armor+canonical_finite_axes")
    ...
    print(f"dominant_route_signature={route_signature!r}")
    ...
    print(f"armor_denominator_proven={armor_catalog.denominator_proven}")
    ...
    for row in scored[: max(1, int(args.top))]:
        value, _key, realization, armor_state, active_bar, payload, unresolved = row
        print(
            f"  value={value:.3f} bar={active_bar} "
            f"armor_types={armor_state.armor_type_count} "
        ...
        active_label = payload.get("resource_active_bar_label")
        if active_label:
            print(f"    active_bar_runtime={active_label!r}")

    print()
    print("BEST ORDINARY MAX STAMINA INCUMBENT")
    print(f"value={best_value:.3f}")
    print(f"route_signature={route_signature!r}")
    ...
    print(
        ...
        "Max Stamina special/runtime named-gear scoring"
    )
    return 0

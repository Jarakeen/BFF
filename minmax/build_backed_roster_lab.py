...
class BuildBackedRosterLab:
    ...
    def __init__(self, database_path: str | Path = DEFAULT_DATABASE) -> None:
        ...
        self._character_resolver = CharacterCapabilityResolver(gear_set_effect_variant_resolver=self._gear_resolver)
    ...
    def available_gear_sets(self, limit: int | None = None) -> tuple[tuple[int, str], ...]:
        ...
        with sqlite3.connect(self.database_path) as db:
            query = "SELECT id, name FROM gear_set ORDER BY name COLLATE NOCASE"
    ...
    def available_skills(self, character_class: CharacterClass | str, limit: int | None = 5000) -> tuple[tuple[int, str], ...]:
        return self._skill_repository.available_skills(character_class, limit)

    def add_player(self, name: str, role: Role, character_class: CharacterClass, gear_set_id: int | None = None, gear_pieces: int = 0, active_bar: BarId = BarId.FRONT, gear_sets: tuple[tuple[int, int], ...] | None = None, skill_ids: tuple[int, ...] = ()) -> BuildBackedPlayer:
        if gear_sets is None:
            gear_sets = ((gear_set_id, gear_pieces),) if gear_set_id is not None and gear_pieces > 0 else ()
        normalized_assignments = tuple((int(set_id), max(0, int(piece_count))) for set_id, piece_count in gear_sets if int(piece_count) > 0)
        ...
        if total_pieces > self.MAX_ARMOR_SLOTS:
            validation_errors.append(f"Mock build has {total_pieces} armor/jewelry set pieces; only {self.MAX_ARMOR_SLOTS} slots are available.")
        if len(normalized_skills) > self.MAX_SKILL_SLOTS:
            validation_errors.append(f"Mock build has {len(normalized_skills)} skills; only {self.MAX_SKILL_SLOTS} active-bar slots are available.")

        if validation_errors:
            player = BuildBackedPlayer(name=name.strip() or f"Mock {role.value.title()}", role=role, character_class=character_class, active_bar=active_bar, gear_sets=tuple((set_id, "", pieces) for set_id, pieces in normalized_assignments), skills=tuple((skill_id, "") for skill_id in normalized_skills), validation_errors=tuple(validation_errors))
        ...
        for set_id, piece_count in normalized_assignments:
            ...
            if not self._gear_resolver.resolve(set_id, piece_count):
                unsupported_sources.append(f"{gear_set.name}: no registered support-effect mapping for the equipped bonus tiers.")
        ...
        if normalized_skills and self.database_path.exists():
            ...
            with sqlite3.connect(self.database_path) as db:
                ...
                for skill_id in normalized_skills:
                    ...
                    if not effects:
                        unsupported_sources.append(f"{skill_name or f'Ability {skill_id}'}: no linked support-effect mapping.")
                    selected_records.append((skill_id, skill_name or f"Unknown skill {skill_id}", is_ultimate, effects))

        active_records = [record for record in selected_records if not record[2]]
        ...
        while len(skill_slots) < self.MAX_SKILL_SLOTS:
            ...
            skill_slots.append(SlottedSkill(skill_id=f"test_filler_{index}", skill_line_id="restoration_staff", is_ultimate=(index == 5)))

        bar = Bar(bar_id=active_bar, main_hand=Weapon(weapon_type=WeaponType.RESTORATION_STAFF), off_hand=None, slots=tuple(skill_slots))
        resolved_build = CharacterBuild(name=name.strip() or f"Mock {role.value.title()}", character_class=character_class, role=role, armor=tuple(armor_pieces), front_bar=bar if active_bar == BarId.FRONT else None, back_bar=bar if active_bar == BarId.BACK else None)
        ...
        player = BuildBackedPlayer(name=resolved_build.name, role=role, character_class=character_class, gear_set_id=first_set[0], gear_set_name=first_set[1], gear_pieces=first_set[2], gear_sets=tuple(resolved_set_metadata), skills=tuple(resolved_skill_metadata), active_bar=active_bar, resolved_effects=resolved_effects, validation_errors=tuple(validation_errors), unsupported_sources=tuple(unsupported_sources))
        self.players.append(player)
    ...
    def capabilities(self):
        ...
        active_bars = {build.name: (BarId.FRONT if build.front_bar is not None else BarId.BACK) for build in characters}
        return RosterCapabilityResolver(character_capability_resolver=self._character_resolver).resolve(characters, active_bars)

    def evaluate(self, requirement_set: EncounterRequirementSet | None = None, evaluator: EncounterEvaluator | None = None) -> EncounterEvaluation:

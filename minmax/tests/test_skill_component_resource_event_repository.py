...
def test_repository_links_color_tagged_current_restore_definition(tmp_path):
    path = tmp_path / "eso.db"
    db = sqlite3.connect(path)
    db.executescript(
        """
        CREATE TABLE skill_rank (id INTEGER PRIMARY KEY, ability_id INTEGER NOT NULL);
        CREATE TABLE ability (ability_id INTEGER PRIMARY KEY, coef_description TEXT);
        INSERT INTO skill_rank VALUES (21, 210);
        INSERT INTO ability VALUES (
            210,
            'Invoke the Huntsman blessing, healing you for |cffffff$1|r Health. You also restore |cffffff12|r% Stamina, increasing by up to |cffffff100|r% based on how high your current Health is. Current Restore: |cffffff$2|r While slotted you gain Major Vitality.'
        );
        """
    )
    db.commit()
    ...
    events = SkillComponentResourceEventRepository(path).resolve(21, 2)

...
class ChampionPointStaticRepository:
    ...
    def resolve(self, name: str, points: int) -> tuple[list[Effect], list[str]]:
        ...
        patterns: tuple[tuple[str, tuple[StatId, ...], EffectUnit], ...] = (
            ...
            (rf"^Increases(?: your)? Max Health by {_VALUE} per stage\.$", (StatId.MAX_HEALTH,), EffectUnit.FLAT),
            (rf"^Increases(?: your)? Max Magicka by {_VALUE} per stage\.$", (StatId.MAX_MAGICKA,), EffectUnit.FLAT),
            (rf"^Increases(?: your)? Max Stamina by {_VALUE} per stage\.$", (StatId.MAX_STAMINA,), EffectUnit.FLAT),

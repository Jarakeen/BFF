...
class EffectMapper:
    ...
    @classmethod
    def to_stats(cls, effect_type: str) -> tuple[StatId, ...]:
        normalized = str(effect_type or "").strip().casefold()
        if normalized in cls.STAT_FANOUT_MAP:
            return cls.STAT_FANOUT_MAP[normalized]
        try:
    ...
    def to_stat(cls, effect_type: str) -> StatId:
        stats = cls.to_stats(effect_type)
        if len(stats) != 1:
            raise ValueError(
                f"Engine stat effect type maps to multiple stats; "
                f"use create_additives(): {effect_type!r}"
            )
        return stats[0]
    ...
    def create_additives(
        ...
        *,
        ...
        resolved_unit = cls.to_unit(unit)
        return [
            Effect(
            ...
            for stat in cls.to_stats(effect_type)
        ]

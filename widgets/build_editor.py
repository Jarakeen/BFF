...
class GearSlotRow(QWidget):
    ...
    def set_trait_choices(self, values):
        current = self.trait_combo.currentText().strip()
        self.trait_combo.blockSignals(True)
        self.trait_combo.clear()
        self.trait_combo.addItems(values)
        if current and self.trait_combo.findText(current, Qt.MatchExactly) >= 0:
            self.trait_combo.setCurrentText(current)
        else:
...
class SkillBarRow(QWidget):
    ...
    def set_class(self, eso_class):
        ...
        if not skills:
            ...
            return
        universal = {
            "Two Handed", "One Hand and Shield", "Dual Wield", "Bow", "Destruction Staff",
            "Restoration Staff", "Heavy Armor", "Medium Armor", "Light Armor", "Fighters Guild",
            "Mages Guild", "Psijic Order", "Soul Magic", "Undaunted", "Assault", "Support",
            "Vampire", "Werewolf",
        }
        class_lines = {
        ...
        for i, combo in enumerate(self.fields):
            ...
            if structured:
                ...
                for name in names:
                    combo.addItem(name)
            if current[i]:
                match = combo.findText(current[i], Qt.MatchExactly)
    ...
    def load(self, skills):
        ...
        for field, value in zip(self.fields, values[:6]):
            match = field.findText(str(value or ""), Qt.MatchExactly)
            field.setCurrentIndex(match if match >= 0 else 0)
...
class BuildEditor(QWidget):
    ...
    def showEvent(self, event):
        ...
        while parent is not None:
            if isinstance(parent, QDialog):
                parent.resize(max(parent.width(), 1600), max(parent.height(), 920))
                break
    ...
    def _update_attribute_limits(self):
        ...
        total = sum(values)
        for spin, value in zip((self.attribute_health, self.attribute_magicka, self.attribute_stamina), values):
    ...
    def _build_identity_card(self):
        ...
        self.build_name = self._line("Build name")
        ...
        self.mundus = self._combo(MUNDUS_CHOICES)

        grid.addWidget(QLabel("Build Name"), 2, 0)
        grid.addWidget(self.build_name, 3, 0)

        affiliation = QWidget()
        affiliation_row = QHBoxLayout(affiliation)
        affiliation_row.setContentsMargins(0, 0, 0, 0)
        affiliation_row.setSpacing(12)
        affiliation_row.addWidget(self.vampire)
        affiliation_row.addWidget(self.werewolf)
        affiliation_row.addStretch()
        grid.addWidget(QLabel("Affliction"), 2, 1)
        ...
        for column, (label, spin) in enumerate(
            (("Health", self.attribute_health), ("Magicka", self.attribute_magicka), ("Stamina", self.attribute_stamina))
        ):
            attribute_grid.addWidget(QLabel(label), 0, column)
        ...
        grid.addWidget(self.mundus, 3, 5)
    ...
    def _set_weapon_row_visible(self, key: str, visible: bool):
        for widget in self._weapon_row_widgets.get(key, ()):
            widget.setVisible(visible)

    def _sync_weapon_offhand_visibility(self, bar: str, *, clear_hidden: bool = False):
        main_key = f"{bar}_main_hand"
        off_key = f"{bar}_off_hand"
        main = self.gear_rows[main_key].value
        off = self.gear_rows[off_key].value
        visible = self._bar_uses_offhand(main, off if not clear_hidden else None)
        if not visible and clear_hidden:
            self.gear_rows[off_key].clear()
        self._set_weapon_row_visible(off_key, visible)

    def _sync_offhand_trait_choices(self, bar: str):
        row = self.gear_rows[f"{bar}_off_hand"]
        row.set_trait_choices(ARMOR_TRAITS if row.type_combo.currentText().strip().casefold() == "shield" else WEAPON_TRAITS)
    ...
    def _build_gear_card(self):
        ...
        specs = [
            ...
            ("Shoulders", "Shoulders", True, False),
            ("Chest", "Chest", True, False),
            ("Hands", "Hands", True, False),
            ("Waist", "Waist", True, False),
            ("Legs", "Legs", True, False),
        ...
        for row, (slot, label, armor, weapon) in enumerate(specs, 1):
            icon = self._gear_icon(slot)
            grid.addWidget(icon, row, 0)
            slot_label = QLabel(label)
            slot_label.setMinimumWidth(98)
            ...
            editor = GearSlotRow(self.set_choices, traits, armor=armor, weapon=weapon, weapon_types=weapon_types)
            ...
            if slot.endswith("off_hand"):
                ...
                self._set_weapon_row_visible(slot, False)

        self.gear_rows["front_main_hand"].type_combo.currentTextChanged.connect(
            lambda *_: self._sync_weapon_offhand_visibility("front", clear_hidden=True)
        )
        self.gear_rows["back_main_hand"].type_combo.currentTextChanged.connect(
            lambda *_: self._sync_weapon_offhand_visibility("back", clear_hidden=True)
        )
        self.gear_rows["front_off_hand"].type_combo.currentTextChanged.connect(
            lambda *_: self._sync_offhand_trait_choices("front")
        )
        self.gear_rows["back_off_hand"].type_combo.currentTextChanged.connect(
            lambda *_: self._sync_offhand_trait_choices("back")
        )
        card.addLayout(grid)
        return card
    ...
    def model(self):
        ...
        return PlayerBuild(
            ...
            BuildName=self.build_name.text().strip(),
    ...
    def load(self, model):
        ...
        weapon_map = {
            "front_main_hand": model.FrontBarWeapon,
            "front_off_hand": model.FrontBarOffHand,
            "back_main_hand": model.BackBarWeapon,
            "back_off_hand": model.BackBarOffHand,
            "Neck": model.Necklace,
            "Ring1": model.Ring1,
            "Ring2": model.Ring2,
        }
        for slot, row in self.gear_rows.items():
            if slot in ARMOR_SLOTS:
                ...
                row.load(weapon_map[slot])

        self._sync_offhand_trait_choices("front")
        self._sync_offhand_trait_choices("back")
        self._sync_weapon_offhand_visibility("front")
        self._sync_weapon_offhand_visibility("back")

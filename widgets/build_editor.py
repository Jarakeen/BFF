...
class GearSlotRow(QWidget):
    ...
    def value(self):
        return GearSlot(
            Set=self.set_combo.currentText().strip(), Set2=self.set2_combo.currentText().strip(),
            Quality=self.quality_combo.currentText().strip(), Trait=self.trait_combo.currentText().strip(),
            Enchant=self.enchant_combo.currentText().strip(), EnchantTier=self.enchant_tier_combo.currentText().strip(),
            Level=self.level_combo.currentText().strip(), Weight=self.weight_combo.currentText().strip(),
        )
...
class SkillBarRow(QWidget):
    ...
    def set_class(self, eso_class):
        ...
        if not skills:
            ...
            self._rebuild()
            return
        universal = {"Two Handed", "One Hand and Shield", "Dual Wield", "Bow", "Destruction Staff", "Restoration Staff", "Heavy Armor", "Medium Armor", "Light Armor", "Fighters Guild", "Mages Guild", "Psijic Order", "Soul Magic", "Undaunted", "Assault", "Support", "Vampire", "Werewolf"}
        class_lines = {
            "Dragonknight": {"Ardent Flame", "Draconic Power", "Earthen Heart"}, "Sorcerer": {"Dark Magic", "Daedric Summoning", "Storm Calling"},
            "Nightblade": {"Assassination", "Shadow", "Siphoning"}, "Templar": {"Aedric Spear", "Dawn's Wrath", "Restoring Light"},
            "Warden": {"Animal Companions", "Green Balance", "Winter's Embrace"}, "Necromancer": {"Bone Tyrant", "Grave Lord", "Living Death"},
            "Arcanist": {"Herald of the Tome", "Soldier of Apocrypha", "Curative Runeforms"},
        }
        filtered = []
        for skill in skills:
            if int(skill.get("is_player") or 0) != 1 or int(skill.get("is_passive") or 0) != 0:
                continue
    ...
    def _rebuild(self):
        ...
        names = [str(s.get("name", "")).strip() for s in structured] if structured else [str(s).strip() for s in self.skill_choices]
...
class CompactCPSlot(QWidget):
    ...
    def _sync(self):
        self.entry = ChampionPointEntry(Name=self.combo.currentText().strip(), Points=str(self.points.value()))
        self.changed.emit()
    ...
    def load_entries(self, entries):
        ...
        for entry in entries or []:
            ...
            for slot in self.slots:
                ...
                if any(isinstance(cp, dict) and str(cp.get("name", "")).strip() == entry.Name.strip() and int(cp.get("discipline_id") or 0) == slot.discipline_id for cp in self.cp_choices):
                    slot.load(entry); break
    @property
...
class BossLoadoutCard(FoundryCard):
    ...
    def __init__(self, skill_choices, parent=None):
        ...
        self.boss_name = QLineEdit(); self.front_bar = SkillBarRow(skill_choices); self.back_bar = SkillBarRow(skill_choices)
        self.food = QLineEdit(); self.potion = QLineEdit(); self.notes = QLineEdit()
        remove = FoundryButton("Remove", role=ButtonRole.DANGER, compact=True)
        remove.clicked.connect(lambda: self.removeRequested.emit(self)); self.set_header_action(remove)
        form = QFormLayout()
        form.addRow("Boss", self.boss_name); form.addRow("Front Bar", self.front_bar); form.addRow("Back Bar", self.back_bar)
        form.addRow("Food", self.food); form.addRow("Potion", self.potion); form.addRow("Notes", self.notes); self.addLayout(form)
    def set_class(self, eso_class): self.front_bar.set_class(eso_class); self.back_bar.set_class(eso_class)
    @property
    def value(self):
        return BossLoadout(BossName=self.boss_name.text().strip(), FrontBarSkills=self.front_bar.value, BackBarSkills=self.back_bar.value, Food=self.food.text().strip(), Potion=self.potion.text().strip(), Notes=self.notes.text().strip())
    def load(self, loadout):
        self.boss_name.setText(loadout.BossName); self.front_bar.load(loadout.FrontBarSkills); self.back_bar.load(loadout.BackBarSkills); self.food.setText(loadout.Food); self.potion.setText(loadout.Potion); self.notes.setText(loadout.Notes)
...
class BuildEditor(QWidget):
    ...
    def __init__(self, race_choices=None, set_choices=None, skill_choices=None, cp_choices=None, food_choices=None, potion_choices=None, parent=None):
        super().__init__(parent)
        self.race_choices = race_choices or []; self.set_choices = set_choices or []; self.skill_choices = skill_choices or []
        self.cp_choices = cp_choices or []; self.food_choices = food_choices or []; self.potion_choices = potion_choices or []
        self.image_path = ""; self._notes = ""; self._boss_cards = []
        self._hydrate_reference_data(); self._build_ui()
    ...
    def _build_ui(self):
        ...
        root.addWidget(self._build_identity_card()); root.addWidget(self._build_gear_card()); root.addWidget(self._build_cp_card()); root.addWidget(self._build_skills_card()); root.addWidget(self._build_boss_card())
    @staticmethod
    ...
    def _build_gear_card(self):
        card = FoundryCard("Gear"); grid = QGridLayout(); grid.setContentsMargins(8, 5, 8, 5); grid.setHorizontalSpacing(6); grid.setVerticalSpacing(3)
        headers = ["", "Slot", "Set 1", "Set 2 (Monster/Backup)", "Quality", "Trait", "Enchantment", "Enchant Tier", "Level", ""]
        ...
        for row, (slot, label, armor) in enumerate(specs, 1):
            grid.addWidget(self._gear_icon(slot), row, 0); slot_label = QLabel(label); slot_label.setMinimumWidth(78); grid.addWidget(slot_label, row, 1)
            traits = ARMOR_TRAITS if armor else JEWELRY_TRAITS if slot in {"Neck", "Ring1", "Ring2"} else WEAPON_TRAITS
            editor = GearSlotRow(self.set_choices, traits, armor=armor); self.gear_rows[slot] = editor
    ...
    def _build_cp_card(self):
        card = FoundryCard("Champion Points"); self.cp_grid = ChampionPointGrid(self.cp_choices); card.addWidget(self.cp_grid); return card
    def _build_skills_card(self):
        card = FoundryCard("Skills & Consumables"); self.front_bar = SkillBarRow(self.skill_choices); self.back_bar = SkillBarRow(self.skill_choices); self._apply_class()
        form = QFormLayout(); form.addRow("Front Bar", self.front_bar); form.addRow("Back Bar", self.back_bar); self.food = self._combo(self.food_choices, True); self.potion = self._combo(self.potion_choices, True)
        consumables = QHBoxLayout(); consumables.addWidget(QLabel("Food")); consumables.addWidget(self.food, 1); consumables.addWidget(QLabel("Potion")); consumables.addWidget(self.potion, 1); form.addRow("Consumables", consumables); card.addLayout(form); return card
    def _build_boss_card(self):
        card = FoundryCard("Boss Alternates"); body = QVBoxLayout(); self.boss_container = QVBoxLayout(); body.addLayout(self.boss_container); actions = QHBoxLayout(); actions.setSpacing(8)
        add_boss = FoundryButton("+ Add Boss Alternate", role=ButtonRole.SECONDARY, compact=True); add_build = FoundryButton("+ Add New Build", role=ButtonRole.SECONDARY, compact=True); save = FoundryButton("Save This Build", role=ButtonRole.PRIMARY, compact=True); cancel = FoundryButton("Cancel", role=ButtonRole.SECONDARY, compact=True)
        add_boss.clicked.connect(self.add_boss_loadout); add_build.clicked.connect(self._handle_add_build); save.clicked.connect(self.saveRequested.emit); cancel.clicked.connect(self.cancelRequested.emit)
        actions.addWidget(add_boss); actions.addStretch(); actions.addWidget(add_build); actions.addWidget(save); actions.addWidget(cancel); body.addLayout(actions); card.addLayout(body); return card
    ...
    def add_boss_loadout(self, loadout=None):
        card = BossLoadoutCard(self.skill_choices); card.set_class(self.eso_class.currentText().strip()); card.removeRequested.connect(self._remove_boss_loadout)
        if loadout: card.load(loadout)
        self._boss_cards.append(card); self.boss_container.addWidget(card); return card
    ...
    def load(self, model):
        ...
        self.vampire.blockSignals(True); self.werewolf.blockSignals(True); self.attribute_health.blockSignals(True); self.attribute_magicka.blockSignals(True); self.attribute_stamina.blockSignals(True)
        self.vampire.setChecked(bool(getattr(model, "Vampire", False))); self.werewolf.setChecked(bool(getattr(model, "Werewolf", False)) and not self.vampire.isChecked()); self.attribute_health.setValue(max(0, min(MAX_ATTRIBUTE_POINTS, int(getattr(model, "AttributeHealth", 0) or 0)))); self.attribute_magicka.setValue(max(0, min(MAX_ATTRIBUTE_POINTS, int(getattr(model, "AttributeMagicka", 0) or 0)))); self.attribute_stamina.setValue(max(0, min(MAX_ATTRIBUTE_POINTS, int(getattr(model, "AttributeStamina", 0) or 0))))
        self.attribute_stamina.blockSignals(False); self.attribute_magicka.blockSignals(False); self.attribute_health.blockSignals(False); self.werewolf.blockSignals(False); self.vampire.blockSignals(False); self._sync_affiliations(); self._update_attribute_limits(); self._apply_class()

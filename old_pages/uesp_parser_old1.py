...
def _extract_dialogue(
    ...
    for block in blocks:
        ...
        # UESP uses paragraph text as the dialogue trigger.
        # Example:
        #   Noxious Sludge attack:
        if block_type == "p":
            current_trigger = text.rstrip(":").strip()
            continue

        # Dialogue lines are <dd> blocks.
        if block_type != "dd":
            continue
...
def _match_dialogue_trigger_to_ability(
    ...
    trigger_words = set(_normalize_text(trigger).split())
    ...
    trigger_words -= ignored_words

    if not trigger_words:
        return None

    # These are contextual dialogue categories, not abilities.
    non_ability_triggers = {
        "idling before combat",
        "group wipe",
    }

    if _normalize_text(trigger) in non_ability_triggers:
        return None
    ...
    for ability in abilities:
        name_words = set(
            _normalize_text(ability.name).split()
        )

        description_words = set(
            _normalize_text(ability.description).split()
        )

        score = 0

        # Strong signal: trigger words appear in the ability name.
        score += len(trigger_words & name_words) * 10

        # Strong signal: trigger words describe the ability.
        score += len(trigger_words & description_words) * 5

        # Check each dialogue line individually.
        for entry in dialogue:
            line_words = set(
                _normalize_text(entry.line).split()
            )
...
class UespParser:
    ...
    def find_boss_links(self, page: UespPage) -> list[str]:
        """Best-effort discovery of boss pages linked from a trial/
        dungeon overview page's boss list. Returns an empty list
        (never a guess) if no such section is found - use --boss
        directly for content this misses."""

        parsed = parse_page_html(page.html)
        blocks = _section(parsed.sections, BOSS_SECTION_HEADINGS) or []
        return [title for _, title in _extract_linked_titles(blocks)]

    def detect_content_type(self, page: UespPage, default: str) -> str:
        """Infer trial/dungeon/arena from the page's own wiki
        categories, falling back to ``default`` if none match."""

...
local function save_json()
    ...
    f:write(string.format('    "Coffee":"%s"\n', office.coffee))
    f:write(string.format('    "CoffeeLevel":"%s"\n', office.coffee_Level))
    f:write(string.format('    "Engineering":"%s"\n', office.engineering))
    f:write(string.format('    "Incidents":"%s"\n', office.incidents))
    f:write(string.format('    "Focus":"%s"\n', office.focus))
    f:write(string.format('    "Location":"%s"\n', office.location))
    f:write(string.format('    "Goal":"%s"\n', office.goal))
    f:write(string.format('    "Mood":"%s"\n', office.mood))
...
local function update_stream_events()
    ...
    if chapter_label ~= "" then
        if auto_chapters_enabled then
            obs.obs_frontend_recording_add_chapter(chapterLabel)
        end
        log_marker(chapterLabel)
    else

...
**D6. MOST Sustained DPS — ~30–35%.** Requires build + deterministic rotation + DoT/proc/cooldown/resource/execute state + simulation duration. This is intentionally downstream of the snapshot Extreme core and depends on Phases 13–14. **Phase 14 handoff now exists:** one saved DD build plus its canonical saved RotationPlan can be evaluated through Combat Simulation against explicit target Health/resistance and normalized into an Extreme sustained-DPS lower-bound record. **Candidate-set comparison also exists:** multiple explicitly supplied saved-build witnesses can be evaluated under the same target assumptions and ranked, but a leader is withheld when any candidate is incomplete, execution horizons differ, or the top value is tied. **Saved-state discovery/search now exists:** the optimizer can discover the complete eligible canonical saved DD/DPS denominator, preserve blocking versus informational exclusions, and compare every eligible saved build/rotation witness under one scenario. This is complete only for saved user state. **Generated structural frontier now exists:** race, legal class route, all 64-point attribute allocations, and active-bar coordinates are deterministically pageable from the canonical Extreme global universe without materializing the whole Cartesian product. **Proof-safe sustained-DPS pruning now exists:** generated branches may be discarded only when an externally proven optimistic DPS ceiling is strictly below a legal incumbent; missing/unproven bounds and tie-capable equal ceilings remain open. **Dynamic refinement inventory now exists:** generated structural candidates can enumerate canonical Mundus, mapped food/drink, modeled armor trait/enchant, and route-filtered active-skill identity axes without fabricating a finished build. **Rotation-ceiling aggregation now exists:** an explicit RotationPlan can receive a proof-safe whole-plan DPS ceiling only when every scheduled DD action has an externally proven optimistic total covering direct, periodic, and triggered consequences within the plan horizon. **Action-bound promotion now exists:** exact canonical action occurrences can become pruning ceilings only when a supplied dominance proof covers every required still-open mutation axis; otherwise the branch remains fail-open. **First concrete mutation-axis dominance producer now exists:** one exact action witness can exhaust the canonical joint Mundus × mapped provisioning grid and prove an absolute action-damage ceiling for those two axes when every combination resolves. **Lazy armor trait/enchant frontier now exists:** sustained-DPS search can preserve the full modeled armor trait/enchant denominator for actually equipped armor slots and page it deterministically without materializing the Cartesian product. **Jewelry and weapon identity frontiers now exist:** sustained-DPS search can lazily enumerate equipped jewelry trait/glyph-family states and weapon trait/enchant-family states without materializing their Cartesian products. Weapon enchant scoring remains runtime-owned. **Champion Point legal-loadout frontier now exists:** all canonical slottable stars can be searched under the four-per-discipline structural rule without materializing the full product; dynamic stars remain evaluator-owned. **Gear-set topology and named physical realization now exist:** generated sustained-DPS search can branch first on canonical active-snapshot count partitions, then expand one branch on demand into exact named-set physical witnesses without materializing the entire gear universe. **Dual-bar named-gear compatibility now exists:** complete topology-branch coverage can be composed into legal front/back gear states with shared body/jewelry identity and canonical bar-access rules preserved. **Gear runtime-semantic inventory now exists:** active set-bonus rows can be classified bar-by-bar as reviewed static, conditional-static, verified runtime, mixed, or unsupported after canonical activation rules. Unsupported rows remain blockers, while verified runtime identities still require exact trigger/uptime simulation. **Generated named-buff gear runtime scoring now reaches Combat Simulation:** hypothetical builds can supply explicit progression, legal dual-bar gear, exact RotationPlans, authoritative runtime history, and target assumptions without being persisted as saved builds. Canonical named gear buffs rebuild exact runtime contexts before damage evaluation. **The first generic timed runtime-stat bridge is now closed:** already-active `weapon_spell_damage` EffectVariants are projected into canonical Weapon Damage + Spell Damage inputs during exact runtime context reconstruction, while unknown identities fail closed. Remaining gear-runtime work is broader reviewed EffectVariant coverage plus integration with action/rotation upper bounds. **Potion selection and passive-rank frontiers now exist:** canonical Alchemy formulas are reduced proof-safely to exact trait families (plus no-potion), while passive ranks are lazily enumerated only over native class lines and explicitly owned combat lines. Potion activation remains runtime-owned and shared skill-line ownership is not invented. **Skill-bar topology and morph legality frontier now exists:** generated sustained-DPS search can lazily enumerate legal empty/partial/full front and back bars, preserve every base/morph alternative, enforce one family per bar, retain legal cross-bar duplication, and require explicit shared-line ownership/equipment context. **Cross-axis generated-candidate composition now exists:** gear materialization, bar access, class/subclass route lines, equipped weapon/armor lines, explicit shared-line ownership, CP, potion, passive ranks, and skill bars can now be bound into one deterministic hypothetical DD candidate without stale frontier snapshots overwriting unrelated state. **Generated RotationPlan seed/cadence families now exist:** for each coherent assembled DD candidate, search can lazily preserve every ordinary-skill ordering, legal starting-bar route, and Light-Attack weave state over an explicit horizon, using canonical semi-static scheduling and optional duration refinement. **Ultimate-choice and anchored potion-cadence policy families now exist:** generated plans can preserve no-Ultimate/front/back explicit choices, project as-soon-as-affordable casts through canonical Ultimate resource evidence, preserve no-potion plus before/after anchored first-use policies, repeat potions at a caller-proven effective cooldown, and hard-check resulting Ultimate/potion legality. Continuous potion offsets and deliberate delayed-Ultimate policies remain open. **Execute and fully charged Heavy Attack policy families now exist:** generated rotations can retain the baseline plus canonical execute upgrades, and can enumerate compatible subsets of explicit safe 1.8-second Heavy Attack windows whose reservation provenance promotes to canonical full-charge completion evidence. The remaining rotation-policy work is encounter-demand/policy variants plus continuous potion offsets and deliberate delayed-Ultimate choices. **Proof-safe generated branch-and-bound coordination now exists:** generated branches can be searched lazily with incumbent updates, forced-open missing bounds, strictly-safe pruning, exact canonical leaf evaluation, shared-horizon enforcement, and separate global-maximum versus unique-leader proof. Canonical rotation ceilings and generated runtime simulation now adapt directly into that coordinator without strengthening evidence. **Partial-branch upper-bound envelopes now exist:** independent proven-safe ceilings for the same generated branch can be intersected by taking the tightest bound, and child branches can inherit a safe parent ceiling without double-counting overlapping mechanics. Missing or heuristic local bounds remain non-authoritative. **The first broad structural sustained-DPS ceiling now exists:** if a branch can prove its maximum damage-bearing action count over the exact horizon and prove an absolute per-action total-damage ceiling covering direct/periodic/triggered consequences, FoundryDock can derive a pruning-safe DPS ceiling without inventing a global ESO action-rate rule. **Generated seed-family action-count proof now exists:** the optimizer can derive the maximum damage-bearing action count for an entire semi-static seed family from the canonical step cycle, exact horizon, legal start routes, and weave states without materializing skill-order permutations. The remaining count inflation is isolated to policies that truly add damage actions, chiefly Ultimate insertion; execute and Heavy Attack are replacement policies and potion is non-damage. **Ultimate-added action-count proof now exists:** the selected generated Ultimate policy can promote canonical UltimateResourceTimeline affordability/reservation capacity into a safe maximum count of added Ultimate damage actions; no-Ultimate proves zero and unresolved cost/generation/resource evidence stays open. Combined with the seed-family count proof, the structural action-count side is now covered for the current policy stack. **Closed-descendant absolute per-action ceilings now exist:** once a partial branch has a proven-complete finite descendant denominator and complete canonical direct/periodic/triggered damage consequences for every scheduled damage action, FoundryDock can promote the largest exact action consequence into a safe absolute per-action ceiling for that branch. This gives narrowing branches a strong local pruning bound without algebraically guessing across ESO stat interactions. **Canonical sustained-DPS dominance-axis composition now exists:** generated search has one mutation-axis vocabulary and can union independent proof coverage without conflating axis coverage with numeric optimistic damage. Unknown or missing axes remain explicit blockers. **The first canonical axis proof producer is wired:** complete joint Mundus × provisioning finite-grid dominance now promotes both canonical `mundus` + `food` coverage together and its proven absolute per-action ceiling. Incomplete grids promote neither half, preserving the coupling discovered earlier. **Gear and progression structural dominance producers are now wired:** complete dual-bar named-gear closure promotes `gear_topology` + `named_gear_realization`; complete Champion Point closure promotes `champion_points`; complete passive-rank closure promotes `passive_ranks`. These are coverage proofs only and deliberately do not claim traits/enchants/runtime/racial or numeric damage dominance. **A generic finite-axis numeric dominance engine now exists:** any proven finite mutation-axis denominator can be exhausted against one exact scheduled action while non-target axes remain fixed; complete same-coordinate canonical consequences promote both axis coverage and the largest exact action total. This avoids duplicating separate proof loops for CP, passive ranks, and gear slices. **Concrete CP/passive/gear frontiers now feed the numeric dominance engine lazily:** complete Champion Point, passive-rank, and dual-bar named-gear denominators expose indexed candidates without eagerly materializing their potentially enormous search spaces. **CP/passive/gear choices now reach canonical action damage:** lazy finite-axis payloads can be adapted onto one fixed witness and delegated to the Combat Simulation DD provider stack for exact occurrence evidence. CP mutates the candidate build loadout, passive ranks mutate explicit generated progression, and dual-bar gear is canonically materialized; unresolved runtime set/proc/periodic/weapon mechanics remain fail-closed. **Reusable CP/passive/gear numeric dominance searches now exist:** each family now has one composition entry point that connects its lazy canonical frontier to the exact-action DD provider and generic finite-axis dominance proof. Callers no longer need to hand-wire frontier + evaluator + proof engine. **Combat-dynamic structural dominance producers are now wired:** complete skill-bar denominators promote `skill_bars`; complete semi-static seed families promote `rotation_order` + `light_attack_weave`; finite anchored Ultimate/potion families promote `ultimate_policy` + `potion_timing_policy`; and reviewed execute/Heavy Attack families promote their own axes. Continuous potion first-use offset, arbitrary delayed-Ultimate timing, encounter-demand policy, and Heavy Attack windows beyond caller-reviewed safe windows remain explicit omitted scope. **Finite whole-plan dynamic dominance now exists:** dynamic families whose plan shape/action identity changes can be exhausted using complete modeled sustained DPS over one exact horizon, promoting the family maximum as a proof-safe pruning ceiling without forcing invalid same-action comparisons. Omitted continuous timing and encounter scope remains separate. **Dynamic whole-plan scoring now reuses the generated Combat Simulation authority:** one fixed build/progression/gear/runtime/target witness can score different legal plan choices through the canonical generated sustained-DPS evaluator while preserving one comparison horizon. **Dynamic sustained-DPS families now connect end to end into whole-plan dominance:** seed rotation, anchored Ultimate/potion, execute, and reviewed Heavy Attack policy frontiers expose lazy indexed choices; one fixed build/progression/gear/runtime/target witness scores them through Combat Simulation; complete same-horizon families can promote a pruning ceiling into branch-and-bound. Continuous potion offset, delayed Ultimate timing, unreviewed Heavy Attack windows, encounter-demand policy, and broader runtime-state search remain explicit omitted scope. **Finite runtime-state proof producers now exist:** an externally proven local runtime family can vary only `ExtremeRuntimeSnapshot`, score each state through generated Combat Simulation at one fixed witness/horizon, and promote a local `runtime_state` ceiling when every choice closes. The engine still does not infer a global proc/cooldown/condition/encounter timeline denominator. **Finite dynamic/runtime-family ceilings now reach branch-and-bound safely:** a scope adapter requires exact equivalence between the finite-family denominator and the branch being pruned, plus explicit exclusion of every omitted-scope item. An integration test drives a closed runtime-state family through whole-plan dominance, branch-scope adaptation, incumbent establishment, and safe branch pruning without expansion. Continuous timing, encounter-demand policy, and unenumerated runtime histories remain open, so final theoretical-maximum proof is still not claimed.

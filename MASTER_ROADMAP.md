...
**D6. MOST Sustained DPS — ~25–30%.** Requires build + deterministic rotation + DoT/proc/cooldown/resource/execute state + simulation duration. This is intentionally downstream of the snapshot Extreme core and depends on Phases 13–14. **Phase 14 handoff now exists:** one saved DD build plus its canonical saved RotationPlan can be evaluated through Combat Simulation against explicit target Health/resistance and normalized into an Extreme sustained-DPS lower-bound record. Global legal build/rotation search and proof of a maximum remain open.

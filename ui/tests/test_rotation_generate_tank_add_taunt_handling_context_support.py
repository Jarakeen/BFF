...
from ui.rotation_generate_tank_role_evidence_support import RotationGenerateTankObligationContext, RotationGenerateTankRoleEvidenceSupport
...
def test_generate_attaches_add_taunt_handling_as_context_only():
    handling = SimpleNamespace(actor_name="Iron Atronach", handling_class="strong_taunt_maintenance_target")
    service = _ContextService((handling,))
    ...
    support = RotationGenerateTankRoleEvidenceSupport(
        ...
        add_taunt_handling_context_service=service,
    )

    support.compose(
        player_build=SimpleNamespace(Role="Tank"),
        evidence_bundle=SimpleNamespace(encounter_id="xalvakka", resource=object(), content_type="trial"),
    )

    assert plan_evidence.tank_add_taunt_handling_context == (handling,)
    assert service.calls == [{"encounter_id": "xalvakka", "responsibilities": (_responsibility(),)}]

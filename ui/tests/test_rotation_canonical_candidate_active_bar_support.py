...
class _Pipeline:
    ...
    def run_effects(self, **kwargs):
        snapshot = SimpleNamespace(candidate_id="active-bar-candidate", plan=self.final_plan)
        self.scorecard = kwargs["scorecard_resolver"](snapshot)
        selected = (
            SimpleNamespace(
                candidate_id="active-bar-candidate",
            ...
            ranked_candidates=(
                SimpleNamespace(
                    ...
                    reasons=(
                        ...
                        else "final-plan active-bar legality failed"
                    ,),
                ),
            ),
        )
    ...
    return RotationCanonicalCandidateSupport(
        ...
        action_slot_service=_EvidenceService(
            RotationSavedBuildActionSlotEvidence(
                slot_requirements=(
                    RotationActionSlotRequirement("Back Heal", ("back",)),
                )
            )
        ),
    )
...
def test_canonical_candidate_rejects_back_bar_cast_without_swap_even_when_skill_is_slotted_there() -> None:
    plan = _plan(
        RotationAction(10.0, 0, RotationActionKind.SKILL, "Back Heal", "back"),
    )
    pipeline = _Pipeline(plan)
    result = _run(_support(pipeline), plan)

    assert pipeline.scorecard is not None
    assert pipeline.scorecard.slot_violations == ()
    assert len(pipeline.scorecard.active_bar_violations) == 1
...
def test_canonical_candidate_accepts_same_time_swap_before_back_bar_cast() -> None:
    plan = _plan(
        RotationAction(10.0, 0, RotationActionKind.BAR_SWAP, None, "back"),
        RotationAction(10.0, 1, RotationActionKind.SKILL, "Back Heal", "back"),
    )
    pipeline = _Pipeline(plan)
    result = _run(_support(pipeline), plan)

    assert pipeline.scorecard.slot_violations == ()
    assert pipeline.scorecard.active_bar_violations == ()
    assert pipeline.scorecard.supplied_obligations_satisfied
    assert result.validation.selectable is True
    assert result.validation.selected_candidate_id == "active-bar-candidate"


def test_canonical_candidate_respects_explicit_back_initial_bar() -> None:
    plan = _plan(
        RotationAction(5.0, 0, RotationActionKind.SKILL, "Back Heal", "back"),
    )
    pipeline = _Pipeline(plan)
    result = _run(_support(pipeline), plan, initial_bar="back")

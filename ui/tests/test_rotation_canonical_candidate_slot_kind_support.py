...
class _Pipeline:
    ...
    def run_effects(self, **kwargs):
        snapshot = SimpleNamespace(candidate_id="slot-kind-candidate", plan=self.final_plan)
        self.scorecard = kwargs["scorecard_resolver"](snapshot)
        selected = (
            SimpleNamespace(
                candidate_id="slot-kind-candidate",
            ...
            ranked_candidates=(
                SimpleNamespace(
                    ...
                    reasons=(
                        ...
                        else "saved-build slot kind legality failed"
                    ,),
                ),
            ),
        )
...
def test_canonical_candidate_rejects_saved_ultimate_scheduled_as_skill() -> None:
    pipeline = _Pipeline(_plan(RotationActionKind.SKILL))
    ...
    violation = pipeline.scorecard.slot_violations[0]
...
def test_canonical_candidate_accepts_saved_ultimate_scheduled_as_ultimate() -> None:
    pipeline = _Pipeline(_plan(RotationActionKind.ULTIMATE))
    result = _run(_support(pipeline), pipeline.final_plan)
    ...
    assert result.validation.selected_candidate_id == "slot-kind-candidate"

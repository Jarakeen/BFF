...
def test_execution_catalog_still_covers_all_31_records_without_unclassified_family() -> None:
    ...
    assert sum(row.status is ExtremeRecordExecutionStatus.READY for row in rows) == 17
    assert sum(row.status is ExtremeRecordExecutionStatus.SPECIALIZED for row in rows) == 1
    assert sum(row.status is ExtremeRecordExecutionStatus.PENDING for row in rows) == 13

...
def _bundle() -> RotationCanonicalEvidenceBundle:
    return RotationCanonicalEvidenceBundle(
        encounter_id="target-state-test",
        encounter_name="Target State Test",
        content_type="trial",
        demands=(),
        options=(),
        requirements=(),
...
def test_stabilized_generate_dd_providers_receive_runtime_target_state(monkeypatch) -> None:
    ...
    def target_resolver(time_seconds, sequence=None):
        assert (time_seconds, sequence) == (4.0, 2)
        return CombatState(active_buffs=("Major Vulnerability",))
    ...
    assert _SkillProvider.calls[0]["runtime_target_combat_state_resolver"] is None
    assert _SkillProvider.calls[1]["runtime_target_combat_state_resolver"] is target_resolver
    assert len(factory.calls) == 2
    assert factory.calls[0]["runtime_target_combat_state_resolver"] is None
    assert factory.calls[1]["runtime_target_combat_state_resolver"] is target_resolver
    assert target_resolver(4.0, 2).has_buff("Major Vulnerability")
